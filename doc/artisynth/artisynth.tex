...
component is an instance of \javaclassx{ModelComponent}, which contains 
...
\javamethodx[ModelComponent]{getName()} will return {\tt null}.
...
\javamethodx[ModelComponent]{getParent()} returns the component's
parent, which is a \javaclassx{CompositeComponent} (Section
...
\javamethodx[ModelComponent]{connectToHierarchy()} is called, and when
it is removed from its parent, its method
\javamethodx[ModelComponent]{disconnectFromHierarchy()} is called.
When either of these methods are called, the component's parent
(as returned by \javamethodx[ModelComponent]{getParent()})
...
\javamethodx[ModelComponent]{notifyParentOfChange()}, which propagates
an appropriate change event up the hierarchy. It will typically do
this by calling the
\javamethodx[CompositeComponent]{componentChanged()} method of the
parent (see Section \ref{CompositeComponentSec}).

The method \javamethodx[ModelComponent]{hasState()} should return {\tt
...
\javaclassx{StructureChangeEvent} for which
\javamethodx[StructureChangeEvent]{stateIsChanged()} returns {\tt true}.
...
When selected, its \javamethodx[ModelComponent]{isSelected()} method
will return {\tt true}. 
...
\begin{sideblock}
{\bf Important:}\\
\javamethodx[ModelComponent]{setSelected()} should only be used by the
...
For convenience, \javaclassx{ModelComponentBase} provides a base
...
\javamethodx[ModelComponent]{getHardReferences()} and
\javamethodx[ModelComponent]{getSoftReferences()} are used to report
...
\javamethodx[ModelComponent]{updateReferences()} method of the
...
\javamethodx{ComponentUtils.isConnected()} to determine which soft
...
\javaclassx{CompositeComponent} is a subinterface of \javaclassx{ModelComponent}
...
\javaclassx{MutableCompositeComponent} is a subinterface of
\javaclassx{CompositeComponent} which allows child components to be
...
A default implementation of \javaclassx{CompositeComponent} is provided
by \javaclassx{CompositeComponentBase}. It is a non-generic class that
...
\javaclassx{ComponentList} is a much more flexible class which
implements \javaclassx{MutableCompositeComponent} and provides for
...
\javaclassx{MutableCompositeComponent} (which includes {\tt
...
\javamethodx[ModelComponent]{setFixed()},
or by adding the component using the 
\javamethodx[ComponentList]{addFixed()} method of {\tt ComponentList}.
\end{sideblock}

The class \javaclassx{ComponentListImpl} is available as an internal
implementation class for constructing instances of either
\javaclassx{CompositeComponent} or
\javaclassx{MutableCompositeComponent}.  It provides most of the
...
for \javaclassx{CompositeComponentBase} or \javaclassx{ComponentList}.
...
A \javaclassx{Model} is a specific \javaclassx{ModelComponent} that can
...
The method can optionally return a \javaclassx{StepAdjustment} object to
...
\javaclassx{StepAdjustment} object to request a change in step size
(Section \ref{AdaptiveSteppingSec}).
...
\setjavabase{artisynth.core.probes}

A \javaclassx{Probe} is an agent that sets model input data, or records
model output data, over a specific window of time. Probes that set
input data are {\it input probes} (\javaclassx{InputProbe}), while those that
record output data are {\it output probes} (\javaclassx{OutputProbe}). Examples
...
\javaclassx{NumericInputProbe} and \javaclassx{NumericOutputProbe}, which are
...
At present, \javaclassx{NumericInputProbe} defines {\tt getState()} and
{\tt setState()} to save and restore any model property values that it
controls. No state is defined for \javaclassx{NumericOutputProbe}.
...
together within a root model (\javaclass[artisynth.core.workspace]{RootModel}), which is the
...
\javaclassx{StepAdjustment} object, which contains a recommended
