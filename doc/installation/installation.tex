...
\item Java 8 (later versions may work but this is not guaranteed)
...
For Java, it is necessary to install a full software development kit,
which comes with a Java compiler. A simple run time
environment (JRE) will not be sufficient. We recommend Java SE
Development Kit 8u241, which can be obtained from Oracle at
\begin{quote}
\href{http://www.oracle.com/technetwork/java/javase/downloads/index.html}%
{http://www.oracle.com/technetwork/java/javase/downloads/index.html}.
\end{quote}
It is necessary to register with Oracle in order to perform the
download.
...
Package Explorer} and selecting {\sf Team > Update} from the context
menu. Subversion updates can also be done 
\ifWindows
...
unzipped directly into {\tt <PROJECT\_DIR>} (as described in
...
\item Finally, from {\bf outside} Eclipse, locate the file
{\tt eclipseSettings.zip} in the project's top \directory{}, and then
unzip this file directly into that \directory{}, so that {\tt .project}
and {\tt .classpath} appear in the top \directory{}.
\ifMacOS
...
section of the dialog.  You will probably want to check {\sf Save
...
\item From {\bf outside} Eclipse, check to see if the file
{\tt eclipseSettings.zip} exists in the project's top \directory{}.
If it does, unzip this file directly into the top \directory{}, so that
{\tt .project} and {\tt .classpath} appear there.
\ifMacOS
MacOS users are strongly encouraged to read  
Section \ref{installingProjectFiles} for details on how to do this.
\else
...
The project files need to be unzipped into the project directory to
enable the project to be loaded into Eclipse.
