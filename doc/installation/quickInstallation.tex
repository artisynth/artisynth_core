...
the new Apple ARM processor implement a compatibility layer (called
Rosetta) that does appear to allow ArtiSynth to run as is.

\item Java should be installed. ArtiSynth will work with the most
recent Java versions, but we recommend using the older Java 8 since
that will also provide compatibility with MATLAB and the Jython
console. You need a full Java Development Kit (JDK), not simply a
...
from Oracle (registration required)
