...
run it immediately by executing the {\tt artisynth.bat} file located
in {\tt \$ARTISYNTH\_HOME\SEP bin} (see Section \ref{artisynthBat}).
...
done using the command {\tt updateArtisynthLibs.bat}, located
in {\tt \$ARTISYNTH\_HOME\SEP bin}. You can execute it
by double-clicking on it in a file-browser, or
...
(Sections \ref{settingWindowsVariables} and \ref{TypicalEnvironment}),
...
checkouts may be obtained using either TortoiseSVN or Cygwin as
...
directory, then it can be compiled from a Cygwin terminal by running {\tt
...
\ref{EnvironmentVariables} and \ref{CygwinEnvironmentSettings}).  In
collections maintained by ArtiSynth, this will be the directory {\tt
...
\item Set {\tt PATH} to {\tt \$ARTISYNTH\_HOME\SEP lib\SEP \ARCH}
...
\SYSTEM (Sections \ref{settingWindowsVariables} and
\ref{TypicalEnvironment}), then they do not need to be set in the
...
A list of \directories, separated by semi-colons ";", which ArtiSynth
...
semi-colons ";", which Java uses to locate its class files. This
...
A list of \directories, separated by semi-colons ";", which the
operating system uses to locate executable programs and
applications. This should be set to include {\tt \$ARTISYNTH\_HOME\SEP
bin} and {\tt \$ARTISYNTH\_HOME\SEP lib\SEP \ARCH}.  This variable
is described in detail in Section \ref{settingWindowsPath}.
...
standard text editor (such as {\tt Notepad}), and add each path on a
separate line. (Strictly speaking, paths only need to be separated by
whitespace, but placing each on a separate line should make the file
more readable.)
