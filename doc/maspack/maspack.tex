...
%\javamethod[maspack.matrix.Matrix3d]{mul()}
...
%\javamethod*[maspack.matrix]{Matrix3d.mul(Matrix3d)}

%\javamethod*{maspack.matrix.Matrix3d.mul(,)}

%\javamethod*{maspack.matrix.Matrix3d.mul(Matrix3dBase,Matrix3dBase)}

%\javamethod*{maspack.matrix.Matrix3d.mulDiagonalLeft(double,double,double)}

%\javamethod*{maspack.matrix.Matrix3d.mulDiagonalLeft(Vector3d)}

%\javamethod{maspack.matrix.Matrix3d.mulDiagonalLeft(,,)}

%\javamethod{maspack.matrix.Matrix3d.mulDiagonalLeft(Vector3d)}

%\renewcommand{\javabase}{maspack}

%\javaclass[matrix]{Matrix3d}

%\javaclass[]{matrix.Matrix3d}

%\javaclass{matrix.Matrix3d}

%\javamethod[matrix.Matrix3d]{mul()}
...
interface \javaclass{HasProperties}:
...
identifier. The \javamethod[HasProperties]{getProperty()} method returns
a \javaclass{Property} handle to the named property, which can in turn be
used to access that property's values or obtain other information
about it. \javamethod[HasProperties]{getAllPropertyInfo()}
returns a \javaclass{PropertyInfoList}
providing information about all the properties associated with the
class.

A \javaclass{Property} handle supplies the following methods:
\begin{lstlisting}
interface Property
{
   Object get(); 
...
A \javaclass{Range} object supplies information about what values a
...
Two common examples of {\tt Range} objects are \javaclass{DoubleInterval} and
\javaclass{IntegerInterval}, which implement intervals of double and integer
...
\javaclass{PropertyInfo} interface, which can be obtained using the
...
information is contained within a \javaclass{PropertyInfoList}:
\begin{lstlisting}
...
\javamethod{PropertyInfo.getDefaultNumericRange()}; otherwise, that method
...
returned by the property's \javamethod{Property.getRange()} method. The
...
lower limits returned by the \javamethod{Property.getRange()} method are
...
implementing the \javaclass{GLRenderable} interface.  Renderable
objects can then be displayed by a \javaclass{GLViewer}, which provides
features such as viewpoint control, clipping planes, and component
selection.

\subsection{Renderables and the viewer}

Any object to be rendered by \javaclass{GLViewer} should implement the
\javaclass{GLRenderable} interface. Renderables can be added or removed from a
...
\javamethod{GLViewer.rerender()}, which causes the viewer to redraw
...
\javamethod[GLRenderable]{prerender()} method of
GLRenderable:
...
\javamethod[RenderList]{addIfVisible()} 
method of \javaclass{RenderList}. For example, if a renderable has two
...
As mentioned above, the \javamethod{RenderList.addIfVisible()} method
only adds renderables to the current render list if they are
``visible''.  Any object implementing \javaclass{GLRenderable} is
visible by default.  However, if the object also implements
\javaclass{HasRenderProps}, then it is determined to be visible only
if the \javaclass{RenderProps} returned by 
\javamethod{HasRenderProps.getRenderProps()} is non-null and the
associated isVisible method returns true.

\subsection{Object Selection}

GLViewer provides support for the mouse-based selection of renderable
components which implement \javaclass{GLSelectable}, a subinterface of
...
\end{lstlisting}
The method \javamethod[GLSelectable]{isSelectable()} should return 
...
\javamethod[GLSelectable]{numSelectionQueriesNeeded()} should return -1
and \javamethod[GLSelectable]{getSelection()} should do nothing.
...
this, use the method \javamethod{GLViewer.enableGLSelectSelection()}.
...
the GLRenderer methods \javamethod[GLRenderer]{beginSelectionQuery()}
and \javamethod[GLRenderer]{endSelectionQuery()}:
\begin{lstlisting}
...
\javamethod[GLRenderer]{beginSubSelection()} and
\javamethod[GLRenderer]{endSubSelection()}, instead of
\javamethod[GLRenderer]{beginSelectionQuery(int)} and 
\javamethod[GLRenderer]{endSelectionQuery()}, as in
\begin{lstlisting}
   void render (GLRenderer renderer, int flags) {
      int qid = 0; // id for selection query
      for (GLSelectable s : components) {
...
instances of \javaclass{GLSelectionListener} with the GLViewer
using the methods
\begin{lstlisting}
...
\javaclass{GLSelectionEvent}. This provides information about all the
...
\end{lstlisting}
\javamethod[GLSelectionEvent]{numSelectedQueries()} returns the number
of queries that resulted in a selection,
\javamethod[GLSelectionEvent]{getModifiersEx()} returns the extended
keyboard modifiers that were in play when the selection was requested,
and \javamethod[GLSelectionEvent]{getFlags()} returns information
...
\javamethod[GLSelectionEvent]{getSelectedObjects()}, which provides an
...
\javamethod[GLSelectable]{getSelection()} for that selection query. As
...
resulting \javaclass{RenderList} to the necessary viewers.
...
by \javamethod[GLViewer]{setExternalRenderList()}. The contents
