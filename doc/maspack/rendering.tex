...
provides a means to address it by shifting the model frame
by a small offset along the eye frame's $z$ axis, using the
method
%
\begin{lstlisting}[]
void addDepthOffset (double zoffset)
\end{lstlisting}
%
A positive offset moves the model frame closer to the eye.  Each unit
of offset equals approximately the distance needed to move by one unit
of depth buffer precision, evaluated at the eye distance to the far
clipping plane.
...
by adding a depth offset (right).}
\label{depthExample:fig}
\end{figure}

The listing below shows an example in which two colored triangles are
drawn on faces of a cube. If no depth offset is added, the triangles
compete for visibility with the underlying cube faces (Figure
\ref{depthExample:fig}, left). To resolve this, the {\tt render()}
method adds a depth offset to move the model frame slighting closer to
the eye when the triangles are drawn. Because
\javamethod[maspack.render.Renderer]{addDepthOffset()} modifies the
model matrix, the operation is wrapped with calls to
\javamethod[maspack.render.Renderer]{pushModelMatrix()} and
\javamethod[maspack.render.Renderer]{popModelMatrix()}
to preserve the model matrix's value.
%
\begin{lstlisting}[]
import java.awt.Color;
import maspack.matrix.Vector3d;
...
...
   ...
   public void render (Renderer renderer, int flags) {
      ...
      // add a depth offset so the triangles will be visible ...
      renderer.pushModelMatrix();
...
%
\begin{sideblock}
The offset is calculated using the eye distance to the far clipping
plane to ensure that it will usually be large enough to prevent
z-fighting.  However, if the far plane distance is significantly
larger than the distance to the rendered objects, this may cause
artifacts which the application may wish to mitigate by using an
offset smaller than one.
\end{sideblock}
