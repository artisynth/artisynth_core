...
\subsection{Running scripts in batch mode}
\label{JythonBatchScripting:sec}

It is possible to run ArtiSynth and execute a Jython script in "batch"
mode, {\it without} starting the GUI or explicitly opening the Jython
console.  This can be useful when running ArtiSynth remotely, or in
parallel on a cluster of machines.

To run a Jython script in batch mode, simply run ArtiSynth from the
