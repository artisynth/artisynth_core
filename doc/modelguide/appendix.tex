...
its columns are both of unit length and mutually
orthogonal, so that
%
\begin{equation}
\R_{BA}^T \, \R_{BA} = \I
...
which a quantity is described, so that ${}^A\v$ and ${}^B\v$
denote $\v$ with respect to frames A and B, respectively.  Given the
definition of $\R_{BA}$ given above, it is fairly straightforward to
show that
%
\begin{equation}
{}^A\v = \R_{BA} \, {}^B\v
...
coordinate frames, A, B, and C, whose orientations are related to each other by
$\R_{BA}$, $\R_{CB}$, and $\R_{CA}$ (Figure
\ref{rotationsABC:fig}).  If we know $\R_{BA}$ and $\R_{CA}$,
...
the affine transform $\X_{BA}$ takes the form
%
\begin{equation}
