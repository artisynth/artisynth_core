...
handling mechanism built into {\tt MechModel}. Collisions are disabled
...
behaviors. The collision manager itself can be accessed by the {\tt
MechModel} method
%
\begin{lstlisting}[]
  getCollisionManager()
...
For convenience, it is also possible to specify default collision
...
and {\tt Collidable.Self}, since individual behaviors are not
...
that is translated and rotated and then set to be non-dynamic
...
FemMeshComp} will use the mesh it contains (although collisions will
...
surface passes through the table and ellipsoid.
...
among the (separate) meshes of its subcollidables.
...
\javaclass[artisynth.core.femmodels]{FemMeshComp}.
When a mesh component is added to an FEM model, using either {\tt
...
intersects with itself as shown on the right.}
...
using its element widgets instead of its surface mesh, to prevent
...
\item {\tt boolean hasDistanceGrid()}
...
If {\tt hasDistanceGrid()} returns {\tt true}, this method returns the
distance grid.
...
which collisions are implemented in ArtiSynth. Knowledge of these
...
contact constraint is assigned to each contact point, with a
...
linear) intersection contours.  The contours are then used to identify
...
to compute a signed distance grid which in turn is used to handle
collisions.
...
drawContactForces} or {\sf drawFrictionForces} being {\tt true}.  The
default value is 0.

\item[negativeForceColor]\mbox{}

A {\tt Color} value which, if non-null, specifies a distinct color to be used
for rendering contact forces whose values are negative; i.e., directed opposite
to the direction of the contact normal. This can only occur for vertex-based
contacts which are enforced by bilateral constraints when the {\sf
bilateralVertexContact} property is {\tt true}, and indicates temporary
contact sticking.
...
This is more robust than storing these references in  member
...
renderingCollidable} property to 1 (line 79); otherwise, the default
...
defined. This gives an accurate estimate of the per-contact area on
...
Returns a map of the contact forces acting on the vertices of the
...
\item {\tt Map<Vertex3d,Double> getContactPressures(int cidx)}

Returns a map of the contact pressures acting on the vertices of
...
through, its elements are made invisible, and it is instead rendered
...
(Figure \ref{passingThroughObjects:fig}, right).
...
By default, friction forces are computed {\it after} the main
...
Contact results in inequality (i.e., unilateral) constraints. Assume you have a
point in contact with a surface. Its velocity v_n in the direction of the
contact surface normal must satisfy

v_n >= 0

which means the point is free to separate from the surface but not move into
it. This is enforced by a reaction force f_n along the normal, which also
satisfies

f_n >= 0

Moreover, f_n is 0 whenever v_n > 0 (since no need to restrain a separating
contact) and v_n is 0 whenever f_n > 0 (the reaction force prevents the point
from moving).

Computing the velocities and reaction forces to satisfy all this is known as a
"complementarity problem", and it's more computationally expensive than
handling regular equality contraints, particularly when there are many contact
points (as there are when doing FE or EFC contact).

To speed things up, the default ArtiSynth method for FE and EFC contact is to
treat the contacts as equality (bilateral) constraints during the velocity
solve, such that v_n = 0, and then afterwards remove any contacts for which the
resulting f_n < 0. This is the behavior when bilateralVertexContact=true,
and while it results in faster computation
