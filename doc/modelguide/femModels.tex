...
\javaclass[artisynth.core.femmodels]{Particle}, the same methods
described in that section for attaching particles to other particles
and frames are available. 

For example, we can attach an FEM node to a rigid body using a
...
{\tt block}) and an FEM node (called {\tt node}) and then adds it to
...
\end{lstlisting}
%
which attach {\tt node2} to {\tt node1}.
...
\subsubsection{Connecting nodes to directly to elements}

Typically, the nodes of FEM models do not align in a way that makes it
...
\end{equation}
%
where $\f_k$ is the force acting on node $k$ due to $\f$.
...
those available in the underlying {\tt PointFem3dAttachment}:
\begin{lstlisting}[]
...
weights, as described in equation (\ref{pointFemForces:eqn}).
...
done by constraining nodal velocities so that the local volume change,
or {\it divergence}, is zero or close to zero. There are generally two
ways to do this:
...
potential field to try and keep the volume constant.

\end{itemize}

Both of these methods operate independently and both can be used
separately or together. Generally speaking, hard incompressibility
...
adjusting the value of the {\it bulk modulus} material property.  As
...
in the literature. However, it tends not to work so well for
...
artificially increased stiffness known as {\it locking}. Because of
...
commonly being known as a {\it hex dominant} mesh). For hex-based
...
causes a zero division and so only values close to 0.5 (such as 0.49)
can be used. 
