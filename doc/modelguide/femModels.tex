...
are in the desired locations inside the FEM, then call one of the functions:
\begin{lstlisting}[]
FemMesh addMesh ( MeshBase mesh );                // creates and returns FemMesh
FemMesh addMesh ( String name, MeshBase mesh );
\end{lstlisting}
The latter method is a convenience routine that also gives the newly embedded
{\tt FemMesh} a name. 
