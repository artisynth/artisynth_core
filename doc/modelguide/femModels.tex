...
\begin{sideblock}
The caveats about color map rendering described in
Section \ref{ContactRendering:sec} apply. In particular, the
resolution of the color map is limited by the resolution of the
collidable meshes, and so the aplication should ensure that these are
sufficiently fine.
\end{sideblock}
...
\fi
\end{center}
\caption{ContactPressureRender showing the contact pressure
as a spherical FEM model falls onto a rectangular one.  The color map
is drawn on the lower (rectangular) model, with redder valuse
indicating greater.}
...
52-56) and a rectangular surface (lines 59-66), and then fixes the end
nodes of the surface (lines 69-74). Surface rendering is enabled for
the surface (line 65), but not for the ball, in order to improve the
...
be drawn on the {\it second} collidable (the surface), we set the {\sf
...
the ball and the surface. The color map range is explicitly set to lie
...
given the same color map and value range that is used to render the
pressure. Finally, default face and line colors for all components in
the model are set at lines 105-106.

To run this example in ArtiSynth, select {\sf All demos > tutorial >
ContactPressureRender} from the {\sf Models} menu. When run, the
FEM models will collide and render the contact pressure on the bottom
object, as shown in Figure \ref{ContactPressureRender:fig}.
