...
Most of the ArtiSynth components described here are
...
The set of nodes belonging to a finite element model can be obtained by the method
%
\begin{methodtable}{0.5}{0.5}
\midline
%
...
%
\begin{center}
\setlengthLaTeXML{\imglength}{1.5in}{1.4in}
...
\javaclass[artisynth.core.materials]{LinearMaterial}, which
...
\javaclass[artisynth.core.materials]{TransverseLinearMaterial}
and
\javaclass[artisynth.core.materials]{AnisotropicLinearMaterial}.
...
\javaclass[artisynth.core.materials]{YeohMaterial},
\javaclass[artisynth.core.materials]{ArrudaBoyceMaterial},
and
\javaclass[artisynth.core.materials]{VerondaWestmannMaterial}.
...
One reason to create an actual reader instance is that it may provide access to
...
\javaclass[\fem]{FemFactory} that handles both mesh 
...
\javaclass[\fem]{FemModel3d} has the methods:
\begin{lstlisting}[]
...
\javaclass[\fem]{FemElement3d} that automatically creates
...
the function 
\javamethod*[\fem]{FemElement3dBase.getNodeCoords()}.
...
All FEM geometries are of type \javaclass[\fem]{FemMeshComp}, 
...
Since
\javaclass[\fem]{FemNode3d} is a subclass of
...
we can attach an FEM node to a rigid body using
...
%
\begin{lstlisting}[]
   mech.addAttachment (new PointParticleAttachment (node1, node2));
\end{lstlisting}
%
or
%
...
\ref{FemBeam:sec}.  The {\tt build()} method then creates 
...
type
\javaclass[\fem]{PointFem3dAttachment}, which
...
this, it computes the ``natural coordinates'' of the
...
estimated coordinates will be used instead. However, it is
...
\javaclass[\fem]{TetGenReader} class.  The model is then
...
\javamethod[\fem]{FemModel3d.findContainingElement(Point3d)}
...
Various proximity queries are available for this task. To find the
...
method \javamethod[\fem.FemModel3d]{isSurfaceNode()}:
%
\begin{lstlisting}[]
   for (FemNode3d n : fem.getNodes()) {
...
specify
a \javaclass[\fem]{FemModel\$ElementFilter} to
...
\begin{sideblock}
Typically, it is preferred to attach a point to an element only if it
...
\javamethod*[\fem.FemModel3d]{findNearestNodes(,)}
...
\javamethod*[\fem.FemModel3d]{findNearestNode(,)}
...
a \javaclass[\fem]{NodeNumberReader}.  For
...
class \javaclass[\fem]{NodeNumberWriter}, which
...
then repositioned using {\tt transformGeometry()} (
...
behavior is best seen by running the model and using the pull
...
\javaclass[\fem]{PointFem3dAttachment} provides
...
%
\methodentry
{\fem.PointFem3dAttachment.setFromNodes(FemNode:A,double:A)}%
{void setFromNodes (FemNode[] nodes, double[] weights)}%
{\ }%
%
\methodentry
{\fem.PointFem3dAttachment.setFromNodes(Collection,VectorNd)}%
{void setFromNodes (Collection<FemNode> nodes, VectorNd weights)}%
{\ }%
%
\methodentry
{\fem.PointFem3dAttachment.setFromNodes(Point3d,FemNode:A)}%
{boolean setFromNodes (Point3d pos, FemNode[] nodes)}%
{\ }%
%
\methodentry
{\fem.PointFem3dAttachment.setFromNodes(Point3d,Collection)}%
...
\begin{lstlisting}[]
   ...
   public void build (String[] args) {
      ...
      // the positive side of the x-z plane. Also, set surface rendering to
      // show strain values.
...
frontmost elements, allowing the strain values to be displayed closer to the
...
render as white spheres ({\tt setAttachedNodesWhite()}, lines 70-71),
...
the {\tt markers} container of their associated FEM model. They can
...
those available in the implementing class {\tt PointFem3dAttachment}:
\begin{methodtable}{0.95}{0.05}
\midline
%
\methodentry
{\fem.FemMarker.setFromFem(FemModel3d)}%
{void setFromFem (FemModel3d fem)}%
{\ }%
%
\methodentry
{\fem.FemMarker.setFromElement(FemElement)}%
{boolean setFromElement (FemElement3d elem)}%
{\ }%
%
\methodentry
{\fem.FemMarker.setFromNodes(FemNode:A,double:A)}%
{void setFromNodes (FemNode[] nodes, double[] weights)}%
{\ }%
%
\methodentry
{\fem.FemMarker.setFromNodes(Collection,VectorNd)}%
{void setFromNodes (Collection<FemNode> nodes, VectorNd weights)}%
{\ }%
%
\methodentry
{\fem.FemMarker.setFromNodes(FemNode:A)}%
{boolean setFromNodes (FemNode[] nodes)}%
{\ }%
%
\methodentry
{\fem.FemMarker.setFromNodes(Collection)}%
...
%
\methodentry
{\fem.FemModel.addMarker(FemMarker,FemElement)}%
{void addMarker (FemMarker mkr, FemElement3d elem)}%
{\ }%
%
\methodentry
{\fem.FemModel.addMarker(FemMarker,FemNode:A,double:A)}%
{void addMarker (FemMarker mkr, FemNode[] nodes, double[] weights)}%
{\ }%
%
\methodentry
{\fem.FemModel.addMarker(FemMarker,Collection,VectorNd)}%
{void addMarker (FemMarker mkr, Collection<FemNode> nodes, VectorNd weights)}%
{\ }%
%
\methodentry
{\fem.FemModel.addMarker(FemMarker,FemNode:A)}%
{boolean addMarker (FemMarker mkr, FemNode[] nodes)}%
{\ }%
%
\methodentry
{\fem.FemModel.addMarker(FemMarker,Collection)}%
...
{\tt FemModel3d} also provides the following additional methods to add markers
and manage its {\tt marker} container:
%
\begin{methodtable}{0.42}{0.58}
\midline
%
\methodentry
{\fem.FemModel3d.addMarker(Point3d)}%
{void addMarker (Point3d pos)}%
{Adds a marker at {\tt pos} (in world coords).}%
%
\methodentry
{\fem.FemModel3d.addMarker(Point3d,boolean)}%
{void addMarker (Point3d pos, boolean project)}%
{Adds a marker; projects to FEM if outside and {\tt project} is {\tt true}.}%
%
\methodentry
{\fem.FemModel.removeMarker(FemMarker)}%
{boolean removeMarker (FemMarker mkr)}%
{Remove the marker {\tt mkr}; return {\tt true} if it was present.}%
%
\methodentry
{\fem.FemModel.clearMarkers()}%
{void clearMarkers ()}%
{Remove all markers.}%
%
\methodentry
{\fem.FemModel.markers()}%
{PointList<FemMarker> markers()}%
{Return the {\tt markers} container.}%
%
\midline
\end{methodtable}


\javaclass[\fem]{FemMarker} is a subclass of \javaclass[\mech]{Point},
and so has the same accessors for querying position and velocity and setting
forces as described in Section \ref{Points:sec}.
...
\javaclass[\fem]{FrameFem3dAttachment}.
Analogously to
\javaclass[\fem]{PointFem3dAttachment}, the
...
\javaclass[\fem]{FrameFem3dAttachment} provides
several methods for explicitly specifying nodes and weights. The
signatures for these include:
\begin{lstlisting}[]
...
\begin{lstlisting}[]
   public void build (String[] args) {
      ...
      // attach the box to the FEM, using all the nodes of elements 22 and 31
...
\subsection{Attached frames}

A \javaclass[\fem]{FemAttachedFrame} is an {\tt AttachedFrame} that is
connected to a \javaclass[\fem]{FemModel3d}, analogous to
\javaclass[\mech]{FrameAttachedFrame} components that are connected to {\tt
Frame}s (Section \ref{frameAttachedFrames:sec}). The attachment is
element-based: the frame's position and orientation track a weighted
combination of the surrounding FEM nodes, using natural coordinates within the
enclosing (or nearest) element. The frame therefore moves and changes
orientation as the FE model deforms.

{\tt FemAttachedFrame} objects can be created using the constructors
%
\begin{methodtable}{0.95}{0.05}
\midline
%
\methodentry
{\fem.FemAttachedFrame.FemAttachedFrame()}%
{FemAttachedFrame ()}%
{\ }%
%
\methodentry
{\fem.FemAttachedFrame.FemAttachedFrame(RigidTransform3d)}%
{FemAttachedFrame (RigidTransform3d TFW)}%
{\ }%
%
\methodentry
{\fem.FemAttachedFrame.FemAttachedFrame(FemElement3dBase,RigidTransform3d)}%
...
\end{methodtable}
%
where {\tt FemAttachedFrame()} creates a default frame with no attachment, {\tt
FemAttachedFrame(TFW)} sets the initial world pose to {\tt TFW}, and {\tt
FemAttachedFrame(elem,TFW)} also attaches the frame to the specified
element. Once created, the nature of the FE attachment can be set or changed
using
%
\begin{methodtable}[\smallskipamount]{0.52}{0.48}
\midline
%
\methodentry
{\fem.FemAttachedFrame.setFromFem(FemModel3d)}%
{void setFromFem (FemModel3d fem)}%
{Attaches to the nearest element in {\tt fem}.}%
%
\methodentry
{\fem.FemAttachedFrame.setFromElement(FemElement3dBase)}%
{void setFromElement (FemElement3dBase elem)}%
{Attaches to {\tt elem} using natural coordinates computed from the
frame's current pose.}%
%
\methodentry
{\fem.FemAttachedFrame.setFromNodes(Collection,VectorNd)}%
{boolean setFromNodes (\brh Collection<FemNode3d> nodes, VectorNd weights)}%
{Attaches to the specified nodes using the given weights.}%
%
\methodentry
{\fem.FemAttachedFrame.setFromNodes(FemNode3d:A,double:A)}%
{boolean setFromNodes (\brh FemNode3d[] nodes, double[] weights)}%
{Attaches to the specified nodes using the given weights.}%
%
\methodentry
{\fem.FemAttachedFrame.setFromNodes(Collection)}%
{boolean setFromNodes (Collection<FemNode3d> nodes)}%
{Attaches to the specified nodes using inverse-distance weighting based
on the frame's current pose.}%
%
\methodentry
{\fem.FemAttachedFrame.setFromNodes(FemNode3d:A)}%
{boolean setFromNodes (FemNode3d[] nodes)}%
{Attaches to the specified nodes using inverse-distance weighting based
on the frame's current pose.}%
%
\midline
\end{methodtable}
%
The {\tt setFromNodes()} methods mirror those available for {\tt FemMarker}
(Section \ref{sec:fem:nodalAttachments}): the first two take explicit weights,
while the last two determine weights automatically using inverse-distance
weighting as described by equation (\ref{invDistWeights:eqn}).

Once created, the attached frame must be added to the {\tt MechModel}
hierarchy. One choice is to add them directly to the FE model to
which they are attached. Each {\tt FemModel3d} maintains an {\tt
attachedFrames} container, which may be managed using the
following {\tt FemModel3d} methods:
%
\begin{methodtable}[\smallskipamount]{0.52}{0.48}
\midline
%
\methodentry
{\fem.FemModel3d.addAttachedFrame(FemAttachedFrame)}%
{void addAttachedFrame (FemAttachedFrame frm)}%
{Adds an existing frame, attaching it to the nearest element if not
already attached to this model.}%
%
\methodentry
{\fem.FemModel3d.addAttachedFrame(FemAttachedFrame,FemElement3dBase)}%
{void addAttachedFrame (\brh FemAttachedFrame frm, FemElement3dBase elem)}%
{Adds an existing frame, attaching it to the specified element.}%
%
\methodentry
{\fem.FemModel3d.addAttachedFrame(RigidTransform3d)}%
{FemAttachedFrame addAttachedFrame (\brh RigidTransform3d TFW)}%
{Creates a new {\tt FemAttachedFrame} located at world pose $\T_{FW}$
and attaches it to the nearest element.}%
%
\methodentry
{\fem.FemModel3d.removeAttachedFrame(FemAttachedFrame)}%
{boolean removeAttachedFrame (FemAttachedFrame frm)}%
{Removes {\tt frm} from the list; returns {\tt true} if present.}%
%
\methodentry
{\fem.FemModel3d.clearAttachedFrames()}%
{void clearAttachedFrames ()}%
{Removes all frames from the {\tt attachedFrames} list.}%
%
\methodentry
{\fem.FemModel3d.getAttachedFrames()}%
{RenderableComponentList<FemAttachedFrame>\brh getAttachedFrames ()}%
{Returns the {\tt attachedFrames} sub-list.}%
%
\midline
\end{methodtable}
%

Alternatively, attached frames can be added to the {\tt MechModel}'s {\tt
frames} container, which is managed using the {\tt MechModel} methods
%
\begin{methodtable}{0.52}{0.48}
\midline
%
\methodentry
{\mech.MechModel.addFrame()}%
{void addFrame (Frame frame)}%
{Adds frame {\tt frame} to the container.}%
%
\methodentry
{\mech.MechModel.removeFrame()}%
...
\subsection{Example: attached frames for an FE model}
\label{FemAttachedFrames:sec}

\begin{figure}[ht]
   \centering
   \setlengthLaTeXML{\imglength}{0.8\textwidth}{0.6\textwidth}
   \includegraphics[width=\imglength]{images/FemAttachedFrames}
   \caption{FemAttachedFrames model loaded into ArtiSynth.}
   \label{fig:fem:femAttachedFrames}
\end{figure}

The following model demonstrates how to add {\tt FemAttachedFrame}s to an
FEM beam:
\lstset{numbers=left}
\lstinputlisting{../../src/artisynth/demos/tutorial/FemAttachedFrames.java}
\lstset{numbers=none}
This example can be found in {\tt artisynth.demos.tutorial.FemAttachedFrames}.
It extends {\tt FemBeam} (Section \ref{FemBeam:sec}), adding five evenly-spaced
{\tt FemAttachedFrame}s along the beam's local $x$-axis (lines 20--27). Each
frame is positioned at a world-coordinate pose computed from the beam's length,
and added to the FEM model using {\tt fem.addAttachedFrame(TFW)}, which
automatically finds and attaches to the nearest element. The frames are
rendered as solid arrow triads with an axis length of 0.12. Finally, the FEM
surface is made semi-transparent (line 29) so the embedded frames are visible.

To run this example in ArtiSynth, select {\sf All demos > tutorial >
FemAttachedFrames} from the {\sf Models} menu. The model should load and
initially appear as in Figure \ref{fig:fem:femAttachedFrames}. Running the
model will cause the beam to fall under gravity, with all attached frames
moving and rotating to track the deformation of the beam.
...
populate it with two FEM beams, {\tt fem1} and {\tt fem2}, using an
...
cause it to drop and flex under gravity, as shown in Figure
...
\javaclass[\fem]{MaterialBundle}s}.  A material
...
\javaclass[\fem]{MuscleBundle}s}. This
...
Adding a \javaclass[\fem]{MaterialBundle} to
...
Finally, a {\tt MaterialBundle} is a renderable component. Setting its
...
elements, it does provide a good example of material bundle usage.
%
...
Next, a {\tt MaterialBundle} is added and used to apply an additional
...
\javaclass[\fem]{FemMuscleModel}, a subclass 
of \javaclass[\fem]{FemModel3d}.  It differs from a basic
...
\javaclass[artisynth.core.materials]{MuscleMaterial}, described
...
are implemented in the class \javaclass[\fem]{MuscleBundle}.
...
A \javaclass[\mech]{MuscleExciter} component enables you to simultaneously
...
for (int i=1; i<fibrePoints.length; i++) {
   Point pNext = fem.addMarker(fibrePoints[i]);
...
\javaclass[\fem]{MuscleElementDesc}.  This object contains
...
\lstinputlisting[firstline=1]{../../src/artisynth/demos/tutorial/ToyMuscleFem.java}
\lstset{firstnumber={1}}
\else
\lstinputlisting[firstline=30]{../../src/artisynth/demos/tutorial/ToyMuscleFem.java}
\fi
\lstset{numbers=none}
%
...
for \javaclass[\fem]{FemMuscleModel}s, muscle bundles
...
All materials are defined in the package {\tt
artisynth.core.materials}.
...
Both the stress and
strain are symmetric $3 \times 3$ matrices,
%
\begin{equation}
\Bsig = \matl 
...
\end{equation}
%
where $\R$ is a (right-handed) rotation matrix and $\P$ is a symmetric
...
\javaclass[artisynth.core.materials]{LinearMaterialBase},
...
is a standard isotropic linear material, which is also the default
...
\subsubsection{TransverseLinearMaterial}
...
\javaclass[maspack.matrix]{Vector2d} objects, while
{\sf direction} is described by a 
\javaclass[maspack.matrix]{Vector3d}. The {\sf direction} property (as well as
...
{\tt TransverseLinearMaterial}s can be created with the following
constructors:
%
\begin{methodtable}{0.6}{0.4}
%\midline
...
Many of the hyperelastic materials described below are {\it
...
\javaclass[artisynth.core.materials]{IncompressibleMaterialBase}.
...
From this definition it follows that the second Piola-Kirchhoff stress
tensor is given by
%
\begin{equation}
...
%
 ...
 \subsubsection{Fung orthotropic material}

\javaclass[artisynth.core.materials]{FungOrthotropicMaterial}
...
At present, the coordinate frame $\R$ is not defined in the material,
but can be specified on a per-element basis using the element method
\javamethod[\fem.FemElement3dBase]{setFrame(Matrix3dBase)}. For example:
%
\begin{lstlisting}[]
   RotationMatrix3d R;
   FemModel3d fem;
   ...
   for (FemElement3d elem : fem.getElements()) {
      ... compute R as required for the element ...
      elem.setFrame (R);
   }
\end{lstlisting}
%
...
{\tt FungOrthotropicMaterial}s can be created with the following constructors:
%
\begin{methodtable}{0.6}{0.4}
%\midline
%
...
{\tt YeohMaterial}s can be created with the following constructors:
%
\begin{methodtable}{0.6}{0.4}
%\midline
%
...
{\tt ArrudaBoyceMaterial}s can be created with the following constructors:
%
\begin{methodtable}{0.6}{0.4}
%\midline
%
...
\subsubsection{Incompressible material}
...
\javaclass[\mats]{FullBlemkerMuscle} (Section \ref{FullBlemkerMuscle:sec}).
...
muscle bundles} within a \javaclass[\fem]{FemMuscleModel}
...
\javaclass[\fem]{MuscleElementDesc} components contained
...
\javaclass[artisynth.core.materials]{GenericMuscle}
is a muscle material whose force term $f_d(\barlam)$
is given by
%
\begin{equation}
...
\javaclass[artisynth.core.materials]{BlemkerMuscle}
...
\javaclass[artisynth.core.materials]{FullBlemkerMuscle}
...
material
\javaclass[\mats]{IncompressibleMaterial}, described in Section
...
\javaclass[artisynth.core.materials]{SimpleForceMuscle}
is a very simple muscle material whose force term $f_d(\barlam)$ is
given by
%
\begin{equation}
...
Stress, strain, and strain energy density can be computed at each node
...
\subsection{Scalar stress/strain measures}
...
%
where 
\javaclassAlt{\fem.FemModel\$StressStrainMeasure}{StressStrainMeasure} is 
...
These quantities can also be queried with the following convenience
methods:
%
\begin{methodtable}{0.45}{0.55}
\midline
...
Strain energy density can be integrated over an FEM model's volume to
...
%
The {\tt FemElement}
method \javamethod[\fem.FemElement]{getStrainEnergy()} returns the
...
{\tt Strain} & color map of the von Mises strain\\
...
{\tt MaxShearStrain} & color map of the maximum shear strain\\
...
rendered at the element center, while {\tt INTEGRATION\_POINT} causes
...
to be drawn at each integration point
...
\javaclass[\fem]{FemCutPlane} and can be created
...
{Creates a cut plane with pose {\tt TPW}.}%
%
\methodentry
