...
are two main types of grid field component:
...
\begin{lstlisting}[]
   ...
   // distance from the origin, as given by the norm of its position
...
indices to identify FEM nodes and elements because they are guaranteed
...
\begin{sideblock}
Because values are assumed to be constant within each element, element
...
\begin{lstlisting}[]
  ...
  double getValue (FemElement3dBase e, int k)        // get value for point
  double getElementValue (int elemNum, int k)        // get value for volume point
  double getShellElementValue (int elemNum, int k)   // get value for shell point

  // vector fields:
  void setValue (FemElement3dBase e, int k, T value) // set value for point
...
A mesh field specifies scalar or vector values for the features of a triangular
mesh. These can be either the vertices (vertex fields) or faces
...
only for triangular polygonal meshes.
...
\begin{sideblock}
Because values are assumed to be constant within each face, face fields
...
values within vertex or face mesh fields.  However, all
...
incompressibility requires evaluating the bulk modulus property at
...
them to buckle (Figure \ref{RadialMuscle:fig}, bottom).
...
of the drawn line segment. The default value is 0, implying that no
vectors are drawn.
...
For {\tt VectorGridField}, when rendering the grid {\it and}
