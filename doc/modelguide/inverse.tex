...
%
\begin{equation}
\phi_m(\a) \equiv \frac{1}{2} \| \v_* - \v^{k+1} \|^2 = 
...
\end{equation*}
%
where $\f_{e0}$ is the force with zero excitations and $\H_e$ is
the force excitation response matrix. The force trajectory can
then be tracked by minimizing
%
\begin{equation}
...
%
\begin{gather}
\min_{\a} \; w_m \phi_m(\a) + w_e \phi_e (\a) + \frac{w_a}{2} \a^T \W_a \a \notag \\
\text{subject to} \quad 0 \le \a \le {\bf 1}.
\label{fullQP:eqn}
\end{gather}
%
...
%
\begin{gather}
\min_{\Delta\a} \; w_m \phi_m(\Delta\a) + w_e \phi_e (\Delta\a) + 
...
set has more degrees of freedom than the target space.
...
A simple example illustrating the steps of
...
\javaclass[\inverse]{InverseManager} class (Section
...
greater prominence in the solution. Exciter weights can be managed with the
following methods:
%
\begin{methodtable}{0.58}{0.42}
\midline
%
\methodentry
{\inverse.TrackingController.getExcitationWeight(ExcitationComponent)}%
{double getExcitationWeight (ExcitationComponent ex)}%
{Returns the weight for exciter {\tt ex}.}%
%
\methodentry
{\inverse.TrackingController.setExcitationWeight(ExcitationComponent,double)}%
{void setExcitationWeight (ExcitationComponent ex, double w)}%
{Sets the weight for exciter {\tt ex}.}% 
%
\midline
\end{methodtable}
%
...
behavior. If $\v_t$ {\it is} set, care should be taken to ensure that it is
...
0.01 being common. The best weight choice will of course depend on the
application.
...
graphs below.  A regularization weight of 10 results in a larger tracking
error.}
...
Figure \ref{InverseMuscleArmL2:fig} (right) shows the model run with a regularization
...
trajectories for the frame's position (i.e., its origin) are shown in Figure
...
the force effector source using {\tt addForceEffectorTarget()} (lines 26-28),
...
which specifies three knot points with a time step of 0.5. Interpolation is set to
...
causes the center particle to be pulled down (Figure \ref{InverseSpringForce:fig},
...
the target's DOFs. The product of the weight with each subweight produces the
...
force is being tracked, but each implements the interface
...
non-{\tt null}, the exciters are added to
...
{\tt artisynth.demos.tutorial.InverseFrameExciterArm},
and the sections of code where it differs
from {\tt InverseMuscleArm} are listed below:
%
\lstset{numbers=left}
\iflatexml
%% Hack: latexml lstinputlisting doesn't handle firstline correctly
...
\iflatexml
%% Hack: latexml lstinputlisting doesn't handle firstline correctly
\lstinputlisting[firstline=43,lastline=51]{../../src/artisynth/demos/tutorial/InverseFrameExciterArm.java}
\else
...
compute the excitation response matrices such as $\H_m$ and $\H_e$
...
Enables debugging messages. Default value is {\tt false}.
...
A \javaclass[\probes]{PositionInputProbe} (Section \ref{PositionProbes:sec})
...
A \javaclass[\probes]{PositionOutputProbe} which records the positions of all
...
A \javaclass[\probes]{PositionOutputProbe} which records the actual position
trajectory $\x(t)$ followed by all motion sources. Its output values are
extracted from the same component properties used by {\tt TARGET\_POSITIONS},
and its rotation representation is given by the {\sf positionProbeRotRep}
property of the controller.

\item[{\tt COMPUTED\_EXCITATIONS}]\mbox{}
...
If the created probe is a {\tt PositionInputProbe} or {\tt
...
\begin{lstlisting}[]
      PositionOutputProbe trackedProbe = 
         (PositionOutputProbe)InverseManager.createOutputProbe ( ... );
\end{lstlisting}
%
...
those that were computed by the controller, there is now a considerable
...
{\tt "source\_positions.txt''} into the
...
\end{tabular}
\end{center}
\caption{Left: {\tt InverseMuscleArm} when run open loop with excitations
...
\ref{InverseControlPanel:fig} shows the panel for {\tt InverseMuscleArm}.
...
{ControlPanel addInversePanel (RootModel root, \brh TrackingController tcon)}%
...
ArtiSynth supplies an inverse kinematic solver, \javaclass[\probes]{IKSolver},
...
An {\tt IKSolver} is created with the following constructors:
%
\begin{methodtable}{0.8}{0.2}
%
\methodentry
{\probes.IKSolver.IKSolver(,)}%
{IKSolver (MechModel mech, Collection<? extends Marker> mkrs)}%
{\ }%
%
\methodentry
{\probes.IKSolver.IKSolver(,,)}%
{IKSolver (MechModel mech, Collection<? extends Marker> mkrs, VectorNd weights)}%
{\ }%
%
\end{methodtable}
%
{\tt mech} is the {\tt MechModel} containing the bodies and constraints, {\tt
mkrs} is a list of markers, and {\tt weights} is an optional set of marker
weight values. If {\tt weights} is not specified, markers are assumed to have a
weight of 1. Each marker is assumed to be either
a \javaclass[\mech]{FrameMarker} or \javaclass[\mech]{GenericMarker} that is
attached to a \javaclass[\mech]{RigidBody}; other marker types will cause an
...
for the solve, or {\tt -1} if the solve did not converge to the required
tolerance.
...
As an example, the following code processes a trajectory of marker positions by
...
{\tt IKSolver} exports a number of methods related to the solution process:
%
\begin{methodtable}{0.45}{0.55}
\midline
%
...
{\tt IKSolver} supplies the following methods that use its {\tt solve()} method
...
instance of \javaclass[maspack.matrix]{RotationRep}, which indicates how
...
These methods transform target marker data into position probes for either
...
inside its {\tt apply()} method. Marker targets would have to be supplied to the
solver in some way, and it would be necessary to ensure that the bodies being
...
Another way to use {\tt IKSolver} in a simulation is to use an
\javaclass[\probes]{IKProbe}, which embeds an {\tt IKSolver}
inside a\pdfbreak 
...
{IKProbe (String name, MechModel mech, Collection<? extends Marker> mkrs,\brh
...
Another way to set {\tt IKProbe} data is to simply copy it from another probe
...
The example model {\tt artisynth.demos.tutorial.IKMultiJointedArm} shows how to
...
\lstinputlisting[firstline=1,lastline=28]{../../src/artisynth/demos/tutorial/IKMultiJointedArm.java}
\lstset{firstnumber={1}}
\else
\lstinputlisting[firstline=15,lastline=42]{../../src/artisynth/demos/tutorial/IKMultiJointedArm.java}
\fi
\lstset{numbers=none}
%
...
\lstinputlisting[firstline=1]{../../src/artisynth/demos/tutorial/IKInverseMuscleArm.java}
\lstset{firstnumber={1}}
\else
\lstinputlisting[firstline=33]{../../src/artisynth/demos/tutorial/IKInverseMuscleArm.java}
\fi
\lstset{numbers=none}
%
...
To filter the marker data (which for this example has been deliberately
...
an unfiltered trajectory, comment out line 53. The model will then appear as in
...
smooth to avoid large transient excitations, particularly at the start and end
...
In the {\tt InverseMuscleArm} example, there is a noticeable transient at the
