...
\ref{AxisRendering:sec}). It has the type
...
constants {\tt X\_IDX}, {\tt Y\_IDX} and {\tt THETA\_IDX}.  For
rendering, the properties {\sf shaftLength} and {\sf shaftRadius}
control the size of a shaft drawn about the rotation axis, using
the {\sf faceColor} rendering property.  A demo is provided by {\tt
...
\end{equation*}
%\
The value and ranges for $x$ and $y$ are exported by the
properties {\sf x}, {\sf y}, {\sf xRange} and {\sf yRange},
and the coordinate indices are defined by the
constants {\tt X\_IDX} and {\tt Y\_IDX}.  A demo is
provided by {\tt artisynth.demos.mech.PlanarJointDemo}.
...
radius 0.1. The default value of {\sf planeSize} is 0, and drawing the
...
\begin{tabular}{c@{\hskip .5in}c}
 \iflatexml
   \includegraphics[width=1.75in]{images/segmentedPlanarnnector}&
