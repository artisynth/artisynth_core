...
computed is determined by its {\sf inertiaMethod} property, which can
be one
...
Inertia is determined implicitly from the surface mesh and 
the body's density.

\item[MASS]\mbox{}

Inertia is determined implicitly from the surface mesh and the
body's mass (which is divided by the mesh volume to determine a density).
...
inertias $\M_k$ of component meshes $k$ for which {\sf hasMass} is
{\tt true}. If the mesh is a {\tt PolygonalMesh}, then $\M_k$ is
computed by its
\javamethodAlt{maspack.geometry.PolygonalMesh.createInertia(double)}%
{createInertia(density)} method, using the {\sf density} property of
its {\tt RigidMeshComp}. Otherwise, if the mesh is a {\tt PointMesh}
or {\tt PolylineMesh}, then $\M_k$ is computed as the sum of the
point-based inertias for each vertex, with the per-vertex mass
computed by dividing the {\sf mass} property by the number of
vertices.
