...
This section details how to build basic multibody-type mechanical models
consisting of particles, springs, rigid bodies, joints, and other
constraints. Most of the ArtiSynth components described here are
...
\section{Points, particles, and springs}
\label{ParticlesAndSprings:sec}

One of the most basic types of mechanical model consists simply of springs
acting on points or particles.

\subsection{Points}
\label{Points:sec}

Points and particles are both instances of the class
\javaclass[artisynth.core.mechmodels]{Point}, and are dynamic
components containing a three-dimensional position state and a corresponding
velocity state. Subclasses of {\tt Point} include 
\javaclass[artisynth.core.mechmodels]{Particle} (discussed below),
as well as {\it markers} (such as
\javaclass[\mech]{FrameMarker} and \javaclass[\fem]{FemMarker}),
which are massless point markers that can be attached to rigid bodies and FE
models. The position and velocity state can be accessed by the following
methods:
%
...
% 
\methodentry
{\mech.Point.getPosition()}%
{Point3d getPosition()}%
{Returns the point's position.}%
%
\methodentry
{\mech.Point.setPosition(Point3d)}%
{void setPosition (Point3d pos)}%
{Sets the point's position.}%
%
\methodentry
{\mech.Point.getVelocity()}%
{Vector3d getVelocity()}%
{Returns the point's velocity.}%
%
\methodentry
{\mech.Point.setVelocity(Vector3d)}%
...
Points also maintain forces, which cause them and/or other
dynamic components in the system to move. Force accessors include:
%
\begin{methodtable}{0.5}{0.5}
\midline
% 
...
{Adds {\tt f} to the point's net force.}%
%
\methodentry
{\mech.Point.getExternalForce()}%
{Vector3d getExternalForce()}%
{Returns the external force acting on the point.}%
%
\methodentry
{\mech.Point.setExternalForce(Vector3d)}%
{void setExternalForce (Vector3d f)}%
{Sets the external force acting on the point to {\tt f}.}%
%
\midline
\end{methodtable}

{\it External forces} are forces explicitly set by the application; once set,
they will remain the same until {\tt setExternalForce()} is called again. {\it
Net forces} are the sum of the external forces plus all other forces arising
from gravity, damping, springs, FE elements, etc. Net forces are zeroed and
recomputed at the start of each simulation step. Applications (and custom
defined force effectors in particular) can add to the net force using {\tt
addForce()}.

\subsection{Particles}

Particles, implemented by
\javaclass[artisynth.core.mechmodels]{Particle}, are
points that contain mass and hence can move and respond to forces
independently. A subclass of {\tt Particle} is
\javaclass[\fem]{FemNode3d}, which describes the nodes
of an FE model (Chapter \ref{FEMModels:sec}).
...
Once created, a particle should be added to the {\tt MechModel}.  The standard
way is to add it to the model's {\tt particles} container, as illustrated by
...
The {\tt particles} container can be managed by the following {\tt MechModel}
methods:
%
\begin{methodtable}{0.6}{0.4}
\midline
%
\methodentry
{\mech.MechModel.addParticle()}%
{void addParticle (Particle p)}%
{Adds particle {\tt p} to the container.}%
%
\methodentry
{\mech.MechModel.removeParticle()}%
{boolean removeParticle (Particle p)}%
{Removes particle {\tt p} from the container.}%
%
\methodentry
{\mech.MechModel.clearParticles()}%
{void clearParticles()}%
{Removes all particles.}%
%
\methodentry
{\mech.MechModel.particles()}%
{PointList<Particle> particles()}%
{Returns the {\tt particles} container.}%
%
%\methodspace{0.5em}%
%\brh
\midline
\end{methodtable}
If desired, applications can also place particles in custom containers
...
\begin{sideblock}
Axial springs connect only two points, but the more
complex \javaclass[\mech]{MultiPointSpring} described in
Chapter \ref{multipointSpringIntro:sec} can be used for more complex paths that
contain via points and obstacle wrapping.
\end{sideblock}
...
\subsection{Axial spring components}

Constructors that create an \javaclass[\mech]{AxialSpring} include:
%
\begin{methodtable}{0.55}{0.45}
\midline
% 
\methodentry
{\mech.AxialSpring.AxialSpring(double,double,double)}%
{AxialSpring (double k, double d, double l0)}%
{Create a spring with a linear material.}%
%
\methodentry
{\mech.AxialSpring.AxialSpring(String,double,double,double)}%
{AxialSpring (String name, double k, double d, double l0)}%
{Create a named spring with a linear material.}%
%
\methodentry
{\mech.AxialSpring.AxialSpring(String,double)}%
...
{\tt k}, {\tt d}, and {\tt l0} specifying stiffness, damping, and rest
length. If not specified, these have default value of $1$, $0$ and $0$.

If a different axial material is needed (Section \ref{AxialMaterials:sec}), the
following methods can be used to query and set the material:
%
\begin{methodtable}{0.55}{0.45}
\midline
% 
\methodentry
{\mech.PointSpringBase.getMaterial()}%
{AxialMaterial getMaterial()}%
{Returns the current material.}%
%
\methodentry
{\mech.PointSpringBase.setMaterial(AxialMaterial)}%
...
Once created, an axial spring must be connected to its points and
added to the {\tt MechModel}. The following methods can
...
% 
\methodentry
{\mech.AxialSpring.setPoints(,)}%
{void setPoints (Point p0, Point p1)}%
{Attaches the spring to points {\tt p0} and {\tt p1}.}%
%
\methodentry
{\mech.AxialSpring.setPoints(,,)}%
{void setPoints (Point p0, Point p1, double l0)}%
{Attaches {\tt p0} and {\tt p1} and sets the rest length.}%
%
\midline
\end{methodtable}
%
The standard way to add a spring to the {\tt MechModel} is
to use the model's {\tt axialSprings} container, which is
managed using the following {\tt MechModel} methods:
%
\begin{methodtable}{0.55}{0.45}
\midline
%
\methodentry
{\mech.MechModel.addAxialSpring()}%
{void addAxialSpring (AxialSpring spr)}%
{Adds spring {\tt spr} to the container.}%
%
\methodentry
{\mech.MechModel.removeAxialSpring()}%
{boolean removeAxialSpring (AxialSpring spr)}%
{Removes spring {\tt spr} from the container.}%
%
\methodentry
{\mech.MechModel.clearAxialSprings()}%
{void clearAxialSprings()}%
{Removes all springs.}%
%
\methodentry
{\mech.MechModel.axialSprings()}%
{RenderableComponentList<AxialSpring> axialSprings()}%
{Returns the {\tt axialSprings} container.}%
%
%\methodspace{0.5em}%
%\brh
\midline
\end{methodtable}
If desired, applications can also place springs in custom containers
(Section \ref{GeneralComponentArrangements:sec}).

{\tt MechModel} also supplies a convenience method to both connect the points
and add the spring to the model (the points themselves must still be added
separately):
%
\begin{methodtable}{0.54}{0.46}
\midline
%
\methodentry
{\mech.MechModel.attachAxialSpring(,,)}%
{void attachAxialSpring (Point p0, Point p1, AxialSpring spr)}%
{Attaches {\tt spr} to {\tt p0} and {\tt p1}
...
A complete application model that implements a simple particle-spring
...
\subsection{Axial materials}
\label{AxialMaterials:sec}

As indicated above, different materials can be supplied to an axial spring in
addition to the default linear material. Most of these are related to modeling
muscles, tendons and ligaments and are presented in detail in
Section \ref{PointToPointMuscles:sec}.

Application authors may also create their own custom axial materials by
subclassing
...
\end{methodtable}
%
where {\tt l} and {\tt l0} are the muscle length and rest length, {\tt ldot} is
$\dot l$, and {\tt excitation} is an additional {\it excitation} signal $a$,
which is usually in the range $[0, 1]$ and is used to implement active springs
...
$\partial f(l, \dot l, a) / \partial \dot l \equiv 0$; i.e., it is
always 0.
...
{\it Frame markers} are markers that can be attached to a
\javaclass[artisynth.core.mechmodels]{Frame} (which includes
\javaclass[artisynth.core.mechmodels]{RigidBody} as a 
subclass), as illustrated in Figure
\ref{frameMarker:fig}. They are frequently used to provide the
anchor points for attaching springs and, more generally, applying forces to the
body.
...
%
\methodentry
{\mech.FrameMarker.getLocation()}%
{Point3d getLocation ()}%
{\ }%
%
\methodentry
{\mech.FrameMarker.setLocation(Point3d)}%
...
%
\methodentry
{\mech.FrameMarker.FrameMarker()}%
{FrameMarker ()}%
{\ }%
%
\methodentry
{\mech.FrameMarker.FrameMarker(String)}%
{FrameMarker (String name)}%
{\ }%
%
\methodentry
{\mech.FrameMarker.FrameMarker(Frame,Point3d)}%
...
\end{methodtable}
%
where {\tt FrameMarker()} creates an empty marker, {\tt FrameMarker(name)}
creates an empty marker with a name, and {\tt FrameMarker(frame,loc)} creates
an unnamed marker attached to {\tt frame} at the location {\tt loc} with
respect to the frame's coordinates. A marker's frame can also be queried and
set with the methods
%
\begin{methodtable}{0.95}{0.05}
\midline
%
\methodentry
{\mech.FrameMarker.setFrame(Frame)}%
{void setFrame (Frame frame)}%
{\ }%
%
\methodentry
{\mech.FrameMarker.getFrame()}%
{Frame getFrame ()}%
{\ }%
%
\midline
...
Once created, a frame marker must be added to the MechModel. This can be
done by adding it to the MechModel's {\tt frameMarkers} container, which is
managed using the following {\tt MechModel} methods:
%
\begin{methodtable}{0.6}{0.4}
\midline
%
\methodentry
{\mech.MechModel.addFrameMarker(FrameMarker)}%
{void addFrameMarker (FrameMarker m)}%
{Adds marker {\tt m} to the container.}%
%
\methodentry
{\mech.MechModel.addFrameMarker(FrameMarker,Frame,Point3d)}%
{void addFrameMarker (FrameMarker m, Frame frame, Point3d loc)}%
{Adds {\tt m} and sets its frame and location.}%
%
\methodentry
{\mech.MechModel.removeFrameMarker(FrameMarker)}%
{boolean removeFrameMarker (FrameMarker m)}%
{Removes marker {\tt m} from the container.}%
%
\methodentry
{\mech.MechModel.clearFrameMarkers()}%
{void clearFrameMarkers()}%
{Removes all markers.}%
%
\methodentry
{\mech.MechModel.frameMarkers()}%
{PointList<FrameMarker> frameMarkers()}%
{Returns the {\tt frameMarkers} container.}%
%
\midline
\end{methodtable}
%
...
%
\methodentry
{\mech.MechModel.addFrameMarker(Frame,Point3d)}%
{FrameMarker addFrameMarker (Frame frame, Point3d loc)}%
{Creates a marker at a frame-relative location.}%
%
\methodentry
{\mech.MechModel.addFrameMarkerWorld(Frame,Point3d)}%
{FrameMarker addFrameMarkerWorld (Frame frame, Point3d locw)}%
{Creates a marker at a world location.}%
%
\midline
\end{methodtable}
%
...
Alternatively, frame markers can also be added directly to the frame or rigid
body to which they are attached. Each frame contains a {\tt markers} container
subcomponent, and markers can be added to this using the following {\tt
Frame} methods:
%
...
%
\methodentry
{\mech.Frame.addMarker(FrameMarker)}%
{void addMarker (FrameMarker mkr)}%
{Adds an existing marker to the frame.}%
%
\methodentry
{\mech.Frame.addMarker(Point3d)}%
{FrameMarker addMarker (Point3d loc)}%
{Creates a marker at location {\tt loc} with respect
to frame coords.}%
%
\methodentry
{\mech.Frame.addMarkerWorld(Point3d)}%
{FrameMarker addMarkerWorld (Point3d pos)}%
{Creates a marker at position {\tt pos} given in world
coordinates.}%
%
\midline
\end{methodtable}
%
A frame's {\tt markers}
container can be accessed, and markers can be removed, using
%
\begin{methodtable}{0.45}{0.55}
\midline
%
\methodentry
{\mech.Frame.getMarkers()}%
{PointList<? extends FrameMarker> getMarkers ()}%
{Returns the frame's {\tt markers} list.}%
%
\methodentry
{\mech.Frame.removeMarker(FrameMarker)}%
{boolean removeMarker (FrameMarker mkr)}%
{Removes a marker from the {\tt markers} list.}%
%
\methodentry
{\mech.Frame.clearMarkers()}%
{void clearMarkers ()}%
{Removes all markers from the {\tt markers} list.}%
%
\midline
\end{methodtable}
...
name} can be specified as {\tt null}.
...
parametric} (Section \ref{ParametricAndAttached:sec}), its pose and
...
is determined by its {\sf inertiaMethod} property, which can be one of:
...
inertia using one of the {\tt setInertia()} methods described above will
...
with non-zero $3 \times 3$ blocks in the lower left and upper right
...
frame, which can be done by setting {\sf faceStyle} to {\tt NONE}
...
As with the rendering properties, the {\sf axisLength} and {\sf
...
A simple application that creates a pair of fixed meshes and adds
...
green by setting its face color; the coordinate frame for the square is
...
\begin{lstlisting}[]
  ...
  double z, thetaDeg; // desired initial coordinate values
  ...
...
\end{lstlisting}
%
and offers a number of methods to directly access coordinate properties:
...
(\ref{MLCPBackward:eqn}) and (\ref{impulsesToForces:eqn})).  The sizes
...
portion of the MLCP in (\ref{MLCPBackward:eqn}), which solves for the
...
\end{equation}
%
Here $\G$ is the system's bilateral constraint matrix, $\tilde\Blam$ denotes
the constraint impulses (from which the constraint forces $\Blam$ can be
determined by $\Blam = \tilde\Blam/h$), the regularization term $\R_g$ is zero,
and for simplicity we have assumed that $\G$ is constant and so the $\b_g$ offset
...
\end{equation}
%
in which $\R_g$ and $\b_g$ contain compliance terms defined by
%
\begin{equation*}
\R_g \equiv 2 \C /h^2, \quad \b_g \equiv - 2 \Bphi^k/h.
\end{equation*}
%
...
(\ref{KKTsoft:eqn}) is {\it regularized} by the $\R_g$.  This means that the
system matrix is always non-singular, even if $\G$ is rank deficient, and so
compliance offers a way to handle overconstrained models, as discussed further
in Section \ref{FourBar:sec}.
...
\end{equation}
%
Once $C_i$ is determined, the damping $D_i$ can be estimated based on
the desired damping ratio $\zeta$, using the formula
%
\begin{equation}
D_i = 2 \zeta \sqrt{M/C_i}
\label{constraintDamping:eqn}
\end{equation}
%
where $M$ is the total mass of the bodies attached to the joint.
...
One possible symptom of an overconstrained system is an error message
...
such a way that the linkage still actually has 1 DOF.
...
properties are set so that its axis is drawn as a blue cylinder.
...
5 & unilateral & enforces limits on $\theta$ \\
\hline
\hline
...
5 & unilateral & enforces limits on the $\theta$ coordinate \\
\hline
\hline
0 & $z$ & translation of $C$ along the $z$ axis \\
...
joint, although the expression for $\T_{CD}$ is now more complicated.
...
coordinate limits are implemented, the joint should be deployed so as to
avoid pitch values near $\pm \pi/2$.
\end{sideblock}
...
4 & unilateral & enforces limits on the $y$ coordinate \\
5 & unilateral & enforces limits on the $\theta$ coordinate \\
\hline
\hline
...
4 & unilateral & enforces limits on the $\theta$ coordinate \\
5 & unilateral & enforces limits on the $\phi$ coordinate \\
\hline
\hline
...
scapulothoracic joints are oriented differently than those of the
...
\begin{lstlisting}[]
  ...
  PlanarConnector (bodyA, bodyB, TDW)
\end{lstlisting}
%
...
\begin{lstlisting}[]
  ...
  SegmentedPlanarConnector (bodyA, bodyB, TDW, segs)
\end{lstlisting}
%
...
ArtiSynth maintains three legacy joints for compatibility with earlier
software:
...
addition, it is usually necessary to also update the wrap paths of any
...
\ref{CoordinatePanels:sec}), and the \javaclass[\gui]{CoordinatePanel}
...
A useful alternative to {\tt LinearFrameMaterial} 
...
enough that the small angle approximation used for {\tt
...
The mesh must be a polygonal mesh composed of triangles.

The points associated with a {\tt PointMeshForce} are added after
...
attachment is created based on the current position of the point
...
\ref{ParametricAndAttached:sec}.  Attachments can be removed by
calling
% method table
\begin{lstlisting}[]
...
first method, the attachment is created based on the current
...
will be in the {\it attached} state, as described in Section
\ref{ParametricAndAttached:sec}.  Frame attachments can be removed by
calling
% method table
\begin{lstlisting}[]
...
\section{Attached frames}
\label{attachedFrames:sec}

In the same sense that a \javaclass[\mech]{Marker} is a point attached to
another dynamic component, an \javaclass[\mech]{AttachedFrame} is a
coordinate frame attached to a dynamic component.

\subsection{Frame attached frames}
\label{frameAttachedFrames:sec}

A \javaclass[\mech]{FrameAttachedFrame} is an {\tt AttachedFrame} that is
rigidly connected to a master \javaclass[\mech]{Frame} (which includes
\javaclass[\mech]{RigidBody} as a subclass), in the same way that a {\tt
FrameMarker} is a point attached to a {\tt Frame}. Once attached, the {\tt
FrameAttachedFrame} moves rigidly with its master frame. The relative pose of
the attached frame with respect to its master is described by the transform
$\T_{FM}$, referred to in the code examples as {\tt TFM}.

{\tt FrameAttachedFrame} objects can be created using a variety of
constructors, including
\begin{methodtable}{0.95}{0.05}
\midline
%
\methodentry
{\mech.FrameAttachedFrame.FrameAttachedFrame()}%
{FrameAttachedFrame ()}%
{\ }%
%
\methodentry
{\mech.FrameAttachedFrame.FrameAttachedFrame(String)}%
{FrameAttachedFrame (String name)}%
{\ }%
%
\methodentry
{\mech.FrameAttachedFrame.FrameAttachedFrame(RigidTransform3d)}%
{FrameAttachedFrame (RigidTransform3d TFW)}%
{\ }%
%
\methodentry
{\mech.FrameAttachedFrame.FrameAttachedFrame(Frame,RigidTransform3d)}%
...
\end{methodtable}
%
where {\tt FrameAttachedFrame()} creates a default frame with no master
assigned, {\tt FrameAttachedFrame(name)} adds a name, and {\tt
FrameAttachedFrame(TFW)} initializes the world pose to {\tt TFW}. The
last constructor also attaches the frame to {\tt frame}, with $\T_{FM}$
...
%
\methodentry
{\mech.FrameAttachedFrame.setFrame(Frame)}%
{void setFrame (Frame frame)}%
{\ }%
%
\methodentry
{\mech.FrameAttachedFrame.getFrame()}%
{Frame getFrame ()}%
{\ }%
%
\midline
\end{methodtable}
%
where {\tt setFrame(frame)} computes $\T_{FM}$ from the attached frame's
current world pose and the master's world pose. The transform $\T_{FM}$ and
the world pose $\T_{FW}$ can also be set and queried directly:
\begin{methodtable}{0.6}{0.4}
\midline
%
\methodentry
{\mech.FrameAttachedFrame.getTFM()}%
{RigidTransform3d getTFM ()}%
{Returns $\T_{FM}$.}%
%
\methodentry
{\mech.FrameAttachedFrame.setTFM(RigidTransform3d)}%
{void setTFM (RigidTransform3d TFM)}%
{Sets $\T_{FM}$ and updates $\T_{FW}$.}%
%
\methodentry
{\mech.FrameAttachedFrame.setTFW(RigidTransform3d)}%
{void setTFW (RigidTransform3d TFW)}%
{Sets $\T_{FW}$ and updates $\T_{FM}$ accordingly.}%
%
\midline
\end{methodtable}
Note there is no need for a {\tt getTFW()} method because that is the same as
the frame's {\tt getPose()} method, whereas {\tt setTFW()} differs from {\tt
setPose(TFW)} because the former updates $\T_{FM}$ to accommodate the new value
of $\T_{FW}$, while the latter does not and so the frame will revert back to
the pose imparted by the master.

Created {\tt FrameAttachedFrame} objects must be added to the {\tt MechModel}.
One way to do this is to add them to the model's {\tt frames} container, which
is managed using the {\tt MechModel} methods:
%
\begin{methodtable}{0.6}{0.4}
\midline
%
\methodentry
{\mech.MechModel.addFrame(Frame)}%
{void addFrame (Frame frame)}%
{Adds frame {\tt frame} to the container.}%
%
\methodentry
{\mech.MechModel.removeFrame(Frame)}%
{boolean removeFrame (Frame frame)}%
{Removes frame {\tt frame} from the container.}%
%
\methodentry
{\mech.MechModel.clearFrames()}%
{void clearFrames()}%
{Removes all frames.}%
%
\methodentry
{\mech.MechModel.frames()}%
{RenderableComponentList<Frame> frames()}%
{Returns the {\tt frames} container.}%
%
%\methodspace{0.5em}%
%\brh
\midline
...
{\tt FrameAttachedFrame} objects can be added directly to a {\tt Frame}
component, which maintains an {\tt attachedFrames} container as
a subcomponent. Attached frames can be added to this list using
%
\begin{methodtable}{0.5}{0.5}
\midline
%
\methodentry
{\mech.Frame.addAttachedFrame(FrameAttachedFrame)}%
{void addAttachedFrame (FrameAttachedFrame frm)}%
{Adds an existing {\tt FrameAttachedFrame} to a frame, 
setting its master to {\tt frame} if necessary.}%
%
\methodentry
{\mech.Frame.addAttachedFrame(RigidTransform3d)}%
{FrameAttachedFrame addAttachedFrame (\brh RigidTransform3d TFM)}%
{Creates a {\tt FrameAttachedFrame} at pose $\T_{FM}$
with respect to frame coordinates.}%
%
\methodentry
{\mech.Frame.addAttachedFrameWorld(RigidTransform3d)}%
{FrameAttachedFrame addAttachedFrameWorld (\brh RigidTransform3d TFW)}%
{Creates a {\tt FrameAttachedFrame} at pose $\T_{FW}$
with respect to world coordinates.}%
%
\midline
\end{methodtable}
%
The {\tt attachedFrames} container can be accessed and frames removed using
%
\begin{methodtable}{0.52}{0.48}
\midline
%
\methodentry
{\mech.Frame.getAttachedFrames()}%
{RenderableComponentList<FrameAttachedFrame>\brh getAttachedFrames ()}%
{Returns the frame's {\tt attachedFrames} container.}%
%
\methodentry
{\mech.Frame.removeAttachedFrame(FrameAttachedFrame)}%
{boolean removeAttachedFrame (\brh FrameAttachedFrame frm)}%
{Removes {\tt frm} from the container; returns {\tt true} if it was present.}%
%
\methodentry
{\mech.Frame.clearAttachedFrames()}%
...
\subsection{Example: attached frames for a rigid body}
\label{RigidBodyAttachedFrames:sec}

\begin{figure}[ht]
\begin{center}
\iflatexml
  \includegraphics[]{images/RigidBodyAttachedFrames}
\else
  \includegraphics[width=3.75in]{images/RigidBodyAttachedFrames}
\fi
\end{center}
\caption{RigidBodyAttachedFrames model loaded into ArtiSynth.}
\label{RigidBodyAttachedFrames:fig}
\end{figure}
...
\end{verbatim}
%
This subclasses {\tt RigidBodyJoint} (Section \ref{RigidBodyJoint:sec}) and
extends its {\tt build()} method to add two {\tt FrameAttachedFrame}s to {\tt
bodyA}, one specified via $\T_{FM}$ and one via the world pose $\T_{FW}$:
%
\lstset{numbers=left}
\begin{lstlisting}[]
   public void build (String[] args) {
      ...
      // center of bodyA's current world pose.
      RigidTransform3d TFW = new RigidTransform3d (bodyA.getPose());
...
\lstset{numbers=none}
%
The first attached frame, {\tt frameAtTip}, is created using {\tt
addAttachedFrame(TFM)}, with $\T_{FM}$ placing it at the tip of {\tt bodyA}
along its local $x$-axis. The second, {\tt frameAtCenter}, is created using
{\tt addAttachedFrameWorld(TFW)}, with its world pose set to match the
current pose of {\tt bodyA}, so it sits at the body's origin. Both frames
are rendered as solid arrow triads ({\tt AxisDrawStyle.ARROW}).

To run this example in ArtiSynth, select {\sf All demos > tutorial >
RigidBodyAttachedFrames} from the {\sf Models} menu. The model should load
and initially appear as in Figure \ref{RigidBodyAttachedFrames:fig}. Running
the model will cause {\tt bodyA} to swing under gravity, with both attached
frames moving rigidly with it.
