...
interface \javaclass[artsiynth.core.util]{TransformableGeometry},
...
{\tt tranformGeometry} can be used to translate, rotate, shear or
...
Each collidable component also provides an
\javamethod[artisynth.core.mechmodels.Collidable]{isCollidable()}
method, which must return {\tt true} to enable collisions involving
that component. For some collidables, the value of this method is tied
to a {\tt collidable} property, so that setting {\tt collidable} to
{\tt false} will disable all collisions involving that component,
regardless of any default and explcit collision behaviors that
have been specified,
...
\ref{FEMModels:sec}) maintain a list of associated meshes (such as
embedded and surface meshes) in a child component list named {\tt
meshes}, and meshes within this list can be made 
collidable.  In general, an ArtiSynth model will contain a collection
of collidables, some of which posses sub-collidables and others which
are solitary (Figure \ref{CollidableGroups:fig}).

\begin{sideblock}
Note: A collidable does not need to be an immediate child component
of a collidale A in order to be a sub-collidable of A;
...
sub-collidables C1 and C2. Self-collisions are allowed among 
a subset of the sub-collidables which are shaded grey.}
\label{CollidableGroups:fig}
\end{figure}

Within a collection of collidables:

\begin{itemize}

\item Actual collisions happen only between leaf components; ancestor
components are used only for grouping purposes.

\item By default, sub-collidables of a component A will not collide
among themselves unless self-collision is enabled for A, in which case
collisions will be enabled among those for which A's 
\javamethod[artisynth.core.mechmodels.Collidable]{allowSelfCollisions()}
method returns {\tt true} (shaded grey in Figure \ref{CollidableGroups:fig}.

\item Self-collision is only possible among the sub-collidables of
A if A is itself deformable; i.e., its
...
\item Specifying a collision behavior among two collidables A and B which
are {\it not} within the same hierarchy will cause that behavior to be
specified among all sub-collidables of A and B.
