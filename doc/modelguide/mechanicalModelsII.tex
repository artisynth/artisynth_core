...
\hline
colorMapping & color mapping properties 
(see Section \ref{TextureMapping:sec}) & {\tt null} \\
normalMapping & normal mapping properties 
(see Section \ref{TextureMapping:sec}) & {\tt null} \\
bumpMapping & bump mapping properties 
(see Section \ref{TextureMapping:sec}) & {\tt null} \\
...
although the last two not available if ArtiSynth is running the OpenGL
2 version of its viewer.

Texture mapping is controlled through the {\sf colorMapping}, {\sf
normalMapping}, and {\sf bumpMapping} properties of {\tt RenderProps}.
These properties have a default value of {\tt null}, but applications
can set them to instances of
\javaclass[maspack.render]{TextureMapProps},
\javaclass[maspack.render]{NormalMapProps}, and
\javaclass[maspack.render]{BumpMapProps}, respectively to provide the
source images and parameters for the associated mapping. All of these
{\tt Props} objects export the following principal properties used by
all mappings:
...
A string giving the file name of the supporting image file.

\end{description}

{\tt NormalMapProps} and {\tt BumpMapProps} also export {\sf
normalScale} and {\sf bumpScale}, respectively, which control normal
and depth ``amplitudes''. Other exported properties control mixing
with underlying colors, and how texture coordinates are both filtered
and managed when they fall outside the canonical range $[0,1]$.  Full
details on texture mapping and its support by the ArtiSynth renderer
are given in the ``Rendering section of the
\href{../maspack/maspack.html}{ Maspack Reference Manual}.

To set up texture mapping, it is often sufficient to set the
appropriate mapping property to a {\tt Props} object for which {\sf
enabled} is set to {\tt true} and {\tt fileName} is set to the file
name of the source image. Normal and bump maps also often require
adjustment of their {\sf normalScale} and {\sf bumpScale} properties.
The following static methods are available for doing this:
%
\begin{lstlisting}
   void RenderProps.setTextureMapProps (Renderable r, TextureMapProps tprops);
   void RenderProps.setTextureEnabled (Renderable r, boolean enabled);
   void RenderProps.setTextureFileName (Renderable r, String fileName);

   void RenderProps.setNormalMapProps (Renderable r, NormalMapProps tprops);
   void RenderProps.setNormalEnabled (Renderable r, boolean enabled);
   void RenderProps.setNormalFileName (Renderable r, String fileName);

   void RenderProps.setBumpMapProps (Renderable r, BumpMapProps tprops);
   void RenderProps.setBumpEnabled (Renderable r, boolean enabled);
   void RenderProps.setBumpFileName (Renderable r, String fileName);
\end{lstlisting}
%

\begin{sideblock}
Texture mapping properties can be set within ancestor nodes of the
component hierarchy, to allow file names and parameters to be
propagated throughout the hierarchy. However, when this is done, it is
still necessary to explicitly make sure that the corresponding mapping
properties within the relevant descendants are non-{\tt null}.  That's
...
a programming idiom for setting up ancestor-controlled mapping
might look like this:
%
\begin{verbatim}
      RenderProps.setTextureMapProps (ancestor, tprops);
      RenderProps.setTextureEnabled (descendant0, true);
      RenderProps.setTextureEnabled (descendant1, true);
\end{verbatim}
%
\end{sideblock}
...
coordinates have been set. This is a non-trivial issue, since finding
texture coordinates that produce appropriate result for a given source
image (the so called ``u-v mapping problem'') is difficult in general
and is highly dependent of the mesh geometry. ArtiSynth users can
handle the problem of assigning texture coordinates in several ways:
...
relevant components. For example, a {\tt RigidBody} can be created
directly from a mesh using
\javamethod*[artisynth.core.mechmodels]{RigidBody.createFromMesh()}.

\item Use a simple mesh object with predefined
texture coordinates. The class \javaclass[maspack.render]{MeshFactory}
...
$(0,0)$ and $(1,1)$ map to the spherical coordinates $(-\pi,\pi)$
(south pole) and $(\pi,0)$ (north pole). Source images are relatively
easy to find for objects with canonical coordinates.

\item Compute custom coordinates and set them within the mesh
using \javamethod*[maspack.geometry.MeshBase]{setTextureCoords()}.

\end{itemize}

A comple example showing texture mapping applied to spherical
meshes to make them look like tennis balls is defined in
%
\begin{verbatim}
...
true}.  The remainder of the {\tt build()} sets up the render
...
the complete data folder name relative to the source directory.
For the bump map, it is important to set the {\sf bumpScale} property
...
and 2. Color mapping is set up by setting color properties in the {\tt
MechModel} itself, but as indicated above, it is still necessary to
also enable color mapping within the individual balls.
