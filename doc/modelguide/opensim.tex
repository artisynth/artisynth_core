...
\item Adjust the visualization and tune parameters if needed so the model is
...
{OpenSimParser (File osimFile, File geometryPath)}% 
{\ }%
%
\end{methodtable}
%
...
on an earlier model by Kate Holzbaur.
%
\lstset{numbers=left}
...
of their C and D coordinate frames. As described in Section
...
\subsection{Interactive control panels}
...
\begin{lstlisting}[]
  ...
  ...
  spr.setNumKnots (segIdx, 30); // increase number of knots to 30
...
the {\tt MultiPointMuscle} {\tt TRIlong} seen in Figure \ref{Arm26Hierarchy2:fig}
...
that allow this to be done directly. However, care must be taken that other
model components either do not have a hard dependency on those being removed,
or that the dependent components are removed as well.  For example, any
...
reference to the object and updates its wrap path.
...
FrameFem3dAttachment} (Section \ref{sec:fem:frameattachments}) for a frame
...
critical point is to choose enough nodes so that stresses induced by frame forces
...
bottom frames are located at the C and D frames of the shoulder and elbow joints,
...
insertion and so is near enough to the FEM model that it can be attached
