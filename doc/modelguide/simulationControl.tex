...
\begin{lstlisting}
   ...
   setStartStopTimes (double t0, double t1);
...
\item {\it One or more properties} which are either set by the numeric
data (input probes), or used to set the the numeric data (output
probes).
...
Each property associated with a numeric probe must have a value
...
%
\begin{lstlisting}
   NumericInputProbe (ModelComponent e, propName, String fileName);
   NumericInputProbe (ModelComponent e, String[] propNames, String fileName);
\end{lstlisting}
%
