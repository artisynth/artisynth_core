...
DQMaxBlendSteps}, which have default values of $10^{-8}$ and $3$.
...
%
\begin{equation*}
w^*_k = \text{exp} \left( -\frac{(d_k - d_\text{min})^2}{2 \sigma^2} \right),
\end{equation*}
%
...
attached point) being skinned, while {\tt nearestPnts} provides
...
equivalent to calling {\tt computeAllVertexConnections()}.
...
by finding and attaching the left and rightmost nodes (lines 28-34).
...
higher resolution than the FEM model (Figure
...
\ref{OverconstrainedContact:sec}), whereby the number of contacts
