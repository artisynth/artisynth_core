...
{\tt maspack.matrix} contains a number of classes that implement rotation
...
\begin{lstlisting}[]
   ...
   setZyxAngles (rz, ry, rx);  // set using Z-Y-X Tait-Bryan angles
   setXyzAngles (rx, ry, rz);  // set using X-Y-Z Tait-Bryan angles
   setZyzAngles (phi, theta, psi); // set using Z-Y-Z Euler angles

   getAxisAngle (axisAng)      // extract the corresponding axis-angle
   getZyxAngles (angles)       // extract the Z-Y-X Tait-Bryan angles
   getXyzAngles (angles)       // extract the X-Y-Z Tait-Bryan angles
   getZyzAngles (angles)       // extract the Z-Y-Z Euler angles
...
\begin{lstlisting}[]
  ...
  PolygonalMesh box = MeshFactory.createBox (10, 20, 30);
...
\item[-1] normals will be written according to the default behavior
described above.
\end{description}
...
\end{lstlisting}
%
Each takes two {\tt PolygonalMesh} objects, {\tt mesh0} and {\tt
mesh1}, and creates and returns another {\tt PolygonalMesh} which
...
giving the number of slices along the circumference, the number of
radial segments on each end cap, and the number of height segments
along the length. The final resulting mesh is shown in Figure
...
At present, source directories will not be found if the reference class
