...
\author{John Lloyd}
\setpubdate{Last update: July, 2021}
\iflatexml
\date{}
\fi
...
\subsubsection{Layout preferences}

Layout preferences control the size and visibility of
...
and not the overall application window containing the viewer.

\item[viewerHeight]\mbox{}

Height of the main ArtiSynth viewer window (in pixels).

\item[timelineVisible]\mbox{}

Whether or not the timeline (Section \ref{TimelineSec}) is visible
when ArtiSynth starts up.

\item[timelineWidth]\mbox{}

Initial width of the timeline (in pixels).

\item[timelineHeight]\mbox{}

Initial height of the timeline (in pixels).
...
files. The {\sf ``Reset default''} button in the lower left corner of the
...
which contains sub-properties (see the ``Composite Properties''
