...
\javaclass{ModelComponent}), some of which may themselves
...
\javaclass{RootModel}), in this case named {\tt Rigid Body Spring}.  The
...
\javaclass{ComponentList}). This provides automatic ``grouping'' of
...
\javaclass{TransformableGeometry} interface. The behavior may also
vary depending on whether or not simulation is in progress.
...
\javaclass{HasCoordinateFrame} interface), then the dragger's initial position
...
renderable. They are defined in the class \javaclass{RenderProps}. 
...
A \javaclass{CompositeProperty} is a Property which contains
...
\item they are \javaclass[artisynth.core.modelbase]{Copyable}
...
A \javaclass[artisynth.core.mechmodels]{FemMarker} is a massless 
\javaclass[artisynth.core.mechmodels]{Point} attached to a
...
The \textsf{Models} menu is automatically created at run-time.  By
default, it has the format described in Section \ref{ModelsMenu}.
...
must be a sub-class of \javaclass{RootModel}.  
...
launch the model \javaclass{FemMuscleArm}.
...
\end{lstlisting}
The previous listing adds every single instance of \javaclass{RootModel} found
