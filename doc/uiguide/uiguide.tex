...
\subsection{Setting aligned axis viewpoints}
\label{axisAlignedViewpointsSec}

\begin{figure}[h]
\begin{center}
\iflatexml
\includegraphics[]{images/alignAxes}
\else
\includegraphics[width=2.5in]{images/alignAxes}
\fi
\end{center}
\caption{A popup menu of aligned axis viewpoints displayed by clicking on the
{\it align axis} button at the lower left of the main ArtiSynth viewer.}
\label{AlignAxes:fig}
\end{figure}

Predetermined viewpoints can also be selected using the {\it align axis} button
located on the viewer control bar.  Clicking on this button produces a popup
menu with icons for different axis-aligned views (Figure \ref{AlignAxes:fig}).
Each view is indicated by the two axes perpendicular to the line of sight, with
the X, Y, and Z axes illustrated by red, green, and blue lines
...
view, and the popup menu shows this view in the upper left corner.  Reselecting
the current view will simply realign the viewer's viewpoint to the current
view; hitting the `{\tt v}' key from within the viewer (Section
\ref{keyShortcutsSec}) will do the same.
...
\subsubsection{Flipping transformer axes forward}

Sometimes, a user may discover that a transformer's frame is
inconveniently oriented, with one or more axes directed away from the
viewer window, obscuring them and making the transformer difficult to
use. This can be mitigated by hitting the 'f' key in the viewer, which will
cause the axes of any active transformer to be flipped so that they
are directed toward the viewer window (Figure
\ref{transformerFlip:fig}).

\begin{figure}[h]
\begin{center}
\begin{tabular}{c c}
\iflatexml
\includegraphics[]{images/transformerUnflipped} &
\includegraphics[]{images/transformerFlipped}
\else
\includegraphics[width=0.3\textwidth]{images/transformerUnflipped} &
\includegraphics[width=0.3\textwidth]{images/transformerFlipped}
\fi
\end{tabular}
\end{center}
\caption{Translational transformer applied to a frame marker,
with axes initially obscured (left) and then brought
forward after hitting the 'f' key (right).}%
\label{transformerFlip:fig}
\end{figure}

\subsubsection{Resizing  transformers}

ArtiSynth will try to automatically set a transformer's dragger
fixture to an appropriate size. In some cases, the user may wish to
change this size, which may be done using the following shortcut keys
in the viewer (while a transformer tool is being used and objects are
selected):

\begin{description}

\item[{\tt CTRL u} or {\tt UP\_ARROW} ]\mbox{}

Increases the transformer size.

\item[{\tt CTRL d} or {\tt DOWN\_ARROW} ]\mbox{}

Decreases the transformer size.

\end{description}

The default size can be restored by hitting the 'u' key (Section
\ref{ChangingTransformerBaseFrame:sec}).
...
If necessary, the pull tool's stiffness and rendering properties can be
adjusted. To do this, right click on tool's button and choose {\sf
``Edit properties ...''}. Properties include:

\begin{description}

\item[renderColor] \mbox{}

Color of the point and arrow rendered by the tool.

\item[radiusRatio] \mbox{}

Radius of the point, as a fraction of the screen width.
...
\label{MeasurementTool}

\vspace{\parskip}
\iflatexml
\phantom{.}\includegraphics[]{images/measurementTool}
\else
\includegraphics[width=.33in]{images/measurementTool}
\fi
{\sf Measurement tool:}\\
Measures distance between two points selected in the viewer.


\begin{figure}[h]
\begin{center}
\iflatexml
\includegraphics[]{images/measuringDistance}
\else
\includegraphics[width=0.6\textwidth]{images/measuringDistance}
\fi
\end{center}
\caption{Measuring the distance between two points in a simple
model of two vertebrae.}%
\label{Measurement:fig}
\end{figure}

Selecting the {\it measurement} tool allows a user to measure distances between
model features. A cross-hair cursor is enabled, with which the user selects two
point locations in the viewer. When the second location is selected, the
distance is displayed in the lower right corner of the viewer (Figure
\ref{Measurement:fig}).

Once two locations are selected, subsequent selection moves the second
location, making it easy to find distances between the first location and
different second locations. Double clicking will clear all locations and allow
the first location to be reselected.

\begin{sideblock}
Locations that can currently be selected by the measurement tool include
points, frames, and any point on a surface mesh. Locations on point-to-point
springs cannot currently be selected but we expect this to implemented soon.
\end{sideblock}

The measurement tool's rendering properties can be
adjusted. To do this, right click on the tool's button and choose {\sf
``Edit properties ...''}. Properties include:
...
Color of the points and line between them.

\item[radiusRatio] \mbox{}

Radius of the points, as a fraction of the screen width.
...
controls, (C) stop button, 
(D) save button, (E) simulation time, 
(F) time cursor, (G) real elapsed tine, 
(H) toolbar, (I) waypoint
track, (J) input track(s), (k) output track(s).}%
\label{timelineFig}
\end{figure}
...
\item[Stop button]\mbox{}

Stops the simulation as well as any running Jython scripts.

\item[Save button]\mbox{}

Saves the data for all output probes with attached files.

\item[Simulation time]\mbox{}

Shows the current simulation time.

\item[Real elapsed time]\mbox{}

Shows the elapsed real time since the start of the simulation.
Used to gauge computation speed.
...
\fi
& Single-step: &
Advances the simulation by a single step, whose size
is specified by the main viewer step size control 
(Figure \ref{PlayControlsFig}).\\
\iflatexml
...
The timeline proper is divided into a number of tracks
(I-K in Figure \ref{timelineFig}). At the top is
...
\subsubsection{Legends and visibility control}

As mentioned above, each entry in a numeric probe's data vector is
rendered in a different color (up to a limit, currently 30, after
which colors are reused).  If the vector size is large (say more than
three or four), or if there is much overlapping of values, then the
result can be difficult to visualize.

To manage this problem, numeric displays provide a {\it legend} dialog that
...
\fi
\end{center}
\caption{Legend dialog for the displays of Figure
\ref{largeDisplayFig}. The label for each entry shows its associated
property and vector element value (in this case, the $x$, $y$ and $z$
values for the {\sf targetPosition} property of the particle {\tt
pnt0}).
}%
\label{legendFig}
\end{figure}

To open the legend dialog, right-click in the display panel and select
{\sf Show legend}. Each row in the legend dialog is associated with a
specific trace, consisting of a descriptive label followed by the
trace color and a toggle controlling its visibility.  For traces
associated with component properties, the label consists of a partial
path identifying the property, and for vector-valued properties this
is followed by the name or number of the corresponding vector
element. Traces are rendered in bottom-to-top order, so those at the
top will be most visible.

\begin{itemize}

\item To change a trace's color, click the {\sf Set} button, which
will bring up a color menu.

\item To make an trace visible or invisible, use the {\sf visible}
toggle box.

\item To make {\it all} traces visible or invisible, use the {\sf All
visible} toggle box at the bottom of the dialog.

\item To change the order in which a trace is drawn, click and drag
its dialog entry vertically.

\end{itemize}

Traces can also be reordered using the {\sf Trace ordering} selector
at the dialog bottom. At the time of this writing, the available
orderings include:
...
Traces are ordered by their natural place in the probe data. This is
the default.

\item[RMS] \mbox{}

Traces are ordered in descending order by the RMS (root mean square)
of their values over time.

\item[RANGE] \mbox{}

Traces are ordered in decreasing order by the difference between their
maximum and minimum values over time.

\end{description}

Reordering traces, and then restricting visibility to a few at the
top, can allow visualization of the most prominent data in an
otherwise crowded display. For example, Figure
\ref{hydrostatExcitations1:fig} shows the 16 muscle excitations
produced by inverse simulation of a muscular hydrostat model, with the
legend dialog shown in Figure \ref{hydrostatLegend:fig}
(left). To view only the five excitations with the largest RMS values,
one can use the {\sf Trace ordering} selector to sort them in
decreasing RMS order, and then make all traces invisible except the
top five. The resulting display is shown in Figure
\ref{hydrostatExcitations2:fig}, with the corresponding legend dialog
shown in Figure \ref{hydrostatLegend:fig} (middle). 

\begin{figure}[h]
\begin{center}
\iflatexml
\includegraphics[]{images/hydrostatExcitations1}
\else
\includegraphics[width=0.60\textwidth]{images/hydrostatExcitations1}
\fi
\end{center}
\caption{Display showing the muscle excitations produced by inverse 
simulation of a muscular hydrostat.}%
\label{hydrostatExcitations1:fig}
\end{figure}

When traces are reordered, their colors are not changed. To reassign
the default colors to the traces based on their current ordering, one
may use the {\sf Reset Colors} button at the bottom of the legend
dialog. Figure \ref{hydrostatExcitations3:fig} shows the result of
doing this for the display of Figure \ref{hydrostatExcitations2:fig},
with the corresponding legend dialog shown in Figure
\ref{hydrostatLegend:fig} (right).

\begin{figure}[h]
\begin{center}
\begin{tabular}{ccc}
\iflatexml
\includegraphics[]{images/hydrostatLegend1} &
\includegraphics[]{images/hydrostatLegend2} &
\includegraphics[]{images/hydrostatLegend3}
\else
\includegraphics[height=4in]{images/hydrostatLegend1} &
\includegraphics[height=4in]{images/hydrostatLegend2} &
\includegraphics[height=4in]{images/hydrostatLegend3}
\fi
\end{tabular}
\end{center}
\caption{Left: legend dialog for Figure
\ref{hydrostatExcitations1:fig}.  Middle: legend dialog for Figure
\ref{hydrostatExcitations2:fig}, where the traces have been sorted in
decreasing RMS order and only the first five traces have been made
visible. Right: legend dialog for Figure
\ref{hydrostatExcitations3:fig}, where the ``{\sf Reset Colors}''
button has been used to reset the trace colors to their canonical
values with respect to the current ordering.}%
\label{hydrostatLegend:fig}
\end{figure}

\begin{figure}[h]
\begin{center}
\iflatexml
\includegraphics[]{images/hydrostatExcitations2}
\else
\includegraphics[width=0.60\textwidth]{images/hydrostatExcitations2}
\fi
\end{center}
\caption{Display showing the five excitations from Figure
\ref{hydrostatExcitations1:fig} with the largest RMS, arranged by
sorting the traces by maximum RMS and then making only the first five
visible.}%
\label{hydrostatExcitations2:fig}
\end{figure}

\begin{figure}[h]
\begin{center}
\iflatexml
\includegraphics[]{images/hydrostatExcitations3}
\else
\includegraphics[width=0.60\textwidth]{images/hydrostatExcitations3}
\fi
\end{center}
\caption{Display showing the excitations from Figure
\ref{hydrostatExcitations2:fig}, with the trace colors reset
to their canonical values with respect to the current ordering.}%
\label{hydrostatExcitations3:fig}
\end{figure}
...
\label{SmoothingData:sec}

Numeric probe data can also be smoothed, which is convenient for
removing noise from either input or output data. To apply data
smoothing, right click in the display and select {\sf ``Smooth data
...''}; this will open a {\it smoothing dialog} as shown in Figure
\ref{smoothingDialog:fig}.

\begin{figure}[h]
\begin{center}
\iflatexml
\includegraphics[]{images/smoothingDialog}
\else
\includegraphics[width=0.3\textwidth]{images/smoothingDialog}
\fi
\end{center}
\caption{Dialog for smoothing probe data.}%
\label{smoothingDialog:fig}
\end{figure}

Different smoothing methods are available and can be selected using
the dialog's {\sf method} field. At the time of this writing, the
following methods are available:

\begin{description}

\item[MOVING\_AVERAGE] \mbox{}
...
\begin{sideblock}
Note that smoothing makes no adjustment to account for uneven knot
spacing, which may sometimes occur for input data.
\end{sideblock}
...
{\sf save} button (D in Figure \ref{timelineFig}).

\subsection{Exporting numeric probe data}
\label{ExportingNumericData:sec}
...
\subsection{Importing numeric probe data}

Numeric probe data can also be imported from either a CSV file ({\tt *.csv}) or
a regular text file ({\tt *.txt}) with formats as described in Section
\ref{ExportingNumericData:sec}. The supplied data should match the probe's
vector size.

To import a probe's data, first select the probe (either within the
navigation panel or by left-clicking on its timeline display bar), and
then right-click and choose {\sf ``Import data ...''}  from the
resulting pull-down menu.

This brings up a dialog (Figure \ref{importProbeDialog:fig}) that
allows the user to specify the file and the following import
parameters:

\begin{description}

\item[Time data included]\mbox{}

A boolean value which if {\tt true} indicates that time values are included
within the file.

\item[Time step]\mbox{}

When time values are not included in the file, this specifies a time step $h$
used to calculate time values from $t = k h$, where $k$ is the line number
starting at 0.
...
\begin{center}
\iflatexml
\includegraphics[]{images/importProbeDialog}
\else
\includegraphics[width=.6\textwidth]{images/importProbeDialog}
\fi
\end{center}
\caption{The import probe data dialog.}%
\label{importProbeDialog:fig}
\end{figure}

Importing data clears all previous probe data. The scaling, start and stop times,
and interpolation methods are not affected.
...
\item[alignDraggersToPoints]\mbox{}

If {\tt true}, and if {\tt initDraggersInWorld} is {\tt false}, then
when a dragger is selected for a collection of point-like objects
which lie either within a plane or along a straight line, the
dragger's orientation will be aligned with the points, as described in
Section \ref{TransformerToolsSec}.  The default value is {\tt false}.
...
necessary to specify this explicitly to avoid its native look and feel.
...
\end{lstlisting}
%
One can also execute a single simulation step using {\tt step()}:
%
\begin{lstlisting}[]
   >> step ()
   >> waitForStop()
\end{lstlisting}
%
As with {\tt play()}, {\tt step()} does not wait for the simulation to
complete. To do that without calling {\tt waitForStop()}, one may use {\tt
stepAndWait()}.
...
\begin{sideblock}
Movies produced by the default method, which creates JPEG-based movie
files, tend to be rather large. If you are making movies regularly,
you should consider installing an external movie making application,
such as {\tt FFmpeg}, and then setting the movie maker to use this via
the {\sf Method} field on the Encoder tab (Section
\ref{EncoderTab:sec}). This will significantly reduce the size of the
movie file and likely improve movie quality. Installing FFMEG is
discussed in Section \ref{InstallingFFmpeg:sec}.
\end{sideblock}
...
Each image file is named {\tt frameXXXXX.zzz}, where {\tt XXXXX} is
the sequential frame number, each {\tt X} is a digit, and {\tt .zzz} is the
image format extension (usually {\tt .png} or {\tt .jpg}).  If {\sf
``Remove temporary files''} is selected in the {\sf Recorder} tab of
the movie panel, the image files will be removed once the movie is
made. If the files are not removed, they will be overwritten by any
subsequent movie creation.
...
FFmpeg to be installed and runnable from the command line (see Section
\ref{InstallingFFmpeg:sec}).
...
Uses the avconv utility, that is sometimes available on linux systems,
...
\label{InstallingFFmpeg:sec}

As described above, movies produced by the default method, which
creates JPEG-based movie files, tend to be rather large. If you are
making movies regularly, you should consider installing an external
movie making application such as {\tt FFmpeg}, and then setting the
movie maker to use this via the {\sf Method} field on the Encoder tab
(Section \ref{EncoderTab:sec}). This will significantly reduce the
size of the movie file and likely improve movie quality.

At the time of this writing, FFmpeg is available for Windows, Mac and
Linux through
\begin{quote}
\href{https://ffmpeg.org/download.html}{ffmpeg.org/download.html}
\end{quote}

The easiest option is to download a precompiled release. The FFmpeg
download site does not provide these itself, but refers to other sites
that do. For example, the Windows download page currently contains a
link to
\begin{quote}
\href{https://www.gyan.dev/ffmpeg/builds}{Windows builds from
gyan.dev}
\end{quote}
which under {\sf git master builds} contains a download link for {\sf
ffmpeg-git-full.7z}. This can be extracted into a location such as
{\tt C:\BKS ffmpeg}, after which the installation's {\tt bin} folder
(e.g., {\tt C:\BKS ffmpeg\BKS bin}) should be added to your Windows
{\tt Path} environment variable. How to do this is described 
in the section ``Adding Directories to the System Path'' in the
Windows version of the ArtiSynth Installation Guide.

Detailed instructions for installing FFmpeg on Windows can also be
found at sites such as
\begin{quote}
\href{https://phoenixnap.com/kb/ffmpeg-windows}{phoenixnap.com/kb/ffmpeg-windows}.
\end{quote}
...
\subsection{Selecting property components}

Double-clicking (using the left mouse button) on the label of a control panel
widget will cause the component hosting that widget's property to be selected.
The path of the selected component will appear in the selection display
beneath the viewer (Section \ref{selectionDisplaySec}), and a context menu can
be invoked for the component by right-clicking (or equivalent) within the
viewer itself.
