...
isCollidable()}, which for many bodies is associated with a {\tt
collidable} property. Setting this property to {\tt false} will
disable collisions for that bodies, regardless of which default and
explicit collisions behaviors are specified.

{\tt Collidable} has also been modified to properly support
self-collisions. A collidable object A can be associated with
sub-collidables, defined by any descendant component of A that is also
collidable. Self-collision can then by effected by enabling
collisions between all pairs of sub-collidables for which A's {\tt
allowSelfCollision()} method returns {\tt true}.
...
getVertexNode(vertex)} (in {\tt FemMesh}).

One change required by this is that {\tt
FemFactory.createHexWedgeExtrusion()} now takes and additional fifth
...
Also: {\tt getSurfaceMeshVertex()} has been replaced by {\tt
getSurfaceVertex()}.
...
PolygonalMesh}. The PolygonalMesh itself can be obtained using {\tt
...
\end{lstlisting}
%
to one where faces are specified by vertex indices and vertices
are specified by their associated nodes and weights, as in:
%
\begin{lstlisting}[]
    [ v 3 1.0
      v 3 0.5 1 0.5
...
\end{lstlisting}
%
This allows the handling of meshes for which each vertex does not
...
FemElement3d}, and {\tt FemModel3d}.  For {\tt FemModel3d},
attachments are created using a default reduction tolerance of 0. If
you want to specify a larger tolerance (and hence possibly reduce the
number of nodes the point is attached to), you can explicitly create
and add the attachment using code like this:
%
\begin{lstlisting}[]
...
To make it easy to get to the body in question, you can now use the
ESC key to quickly select the parent of the last selected component.
For surface meshes, two quick presses of ESC will get you from the
mesh component to the body in question (with the revised selection
showing up in the component selection box at the bottom of the main
ArtiSynth frame). While it was already possible to do parent selection
using the "UP" arrow in the component selection box, using the ESC key
is faster.

\subsection*{New createSurface methods for FemMesh}
