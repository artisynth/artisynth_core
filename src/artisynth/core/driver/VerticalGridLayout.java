...
/**
 * The <code>VerticalGridLayout</code> class is a layout manager that lays out
 * a container's components in a rectangular grid. The container is divided into
 ...
 * shows buttons 5 then 6."> </td>
 * 
 * <td ALIGN=CENTER><img SRC="doc-files/VerticalGridLayout-2.gif" alt="Shows 6
 * buttons in rows of 2. Row 1 shows buttons 2 then 1. Row 2 shows buttons 4
 * then 3. Row 3 shows buttons 6 then 5."> </td>
 * </tr>
 * 
 * <tr ALIGN=CENTER>
 ...
 * Instead, the number of columns is determined from the specified number or
...
public class VerticalGridLayout implements LayoutManager, java.io.Serializable {
    ...
    * columns can be changed at any time. This should be a non negative integer,
    ...
    * if the value of both <code>rows</code> and <code>cols</code> is set to
    * zero
    */
    ...
    * layout.
    * <p>
    * The preferred width of a grid layout is the largest preferred width of any
    ...
    * any of the components in the container times the number of rows, plus the
    ...
    * order to satisfy the constraints of the <code>VerticalGridLayout</code>
    * object.
    * <p>
   ...
   public void layoutContainer (Container parent) {
      synchronized (parent.getTreeLock()) {
         ...
         if (ltr) {
            ...
            // for (int c = 0, x = parent.width - insets.right - w; c < ncols ;
            // c++, x -= w + hgap) {
            for (int c = 0, x = parent.getSize().width - insets.right - w; c < ncols; c++, x -=
               w + hgap) {
               for (int r = 0, y = insets.top; r < nrows; r++, y += h + vgap) {
                  // int i = r * ncols + c;
                  int i = c * nrows + r;
