...
public class ConstraintForceTerm extends LeastSquaresTermBase {
   ...
   public static PropertyList myProps =
      new PropertyList(ConstraintForceTerm.class, LeastSquaresTermBase.class);
   ...
   public ConstraintForceTerm (String name) {
      this ();
      setName (name);
   }
   ...
   public void updateTargetForce (double t0, double t1) {
      ...
      int idx = 0;
      for (ConstraintForceTarget target : myForceTargets) {
   ...
   private void createForceJacobian (MechModel mech) {
      ...
      int targetIdx = 0;
      for (ConstraintForceTarget ft : myForceTargets) {
         int connectorIdx = 0;
      ...
      for (int i = 0; i < myForceTargets.size (); i++) {
         ConstraintForceTarget target = myForceTargets.get (i);
   ...
   /**
    * @deprecated Use {@link #addTarget(BodyConnector)} instead.
    */
   public ForceTarget addForceTarget(BodyConnector con) {
      return (ForceTarget)addTarget (con);
   }

   /**
    * @deprecated Use {@link #addTarget(BodyConnector,double)} instead.
    */
   public ForceTarget addForceTarget(BodyConnector con, double weight) {
      return (ForceTarget)addTarget (con, weight);
   }
   
   /**
    * @deprecated Use {@link #addTarget(BodyConnector,VectorNd)} instead.
    */
   public ForceTarget addForceTarget(BodyConnector con, VectorNd targetLambda) {
      return (ForceTarget)addTarget (con, targetLambda);
   }
   
   /**
    * @deprecated Use {@link #addTarget(BodyConnector,VectorNd,double)} instead.
    */
   public ForceTarget addForceTarget (
      BodyConnector con, VectorNd targetLambda, double weight) {
      return (ForceTarget)addTarget (con, targetLambda, weight);
   }
   
   /**
    * @deprecated Use {@link removeTarget(BodyConnector)} instead.
    */
   protected boolean removeForceTarget(ConstraintForceTarget forceTarget) {
    ...
    */
   public ConstraintForceTarget addTarget(BodyConnector con) {
      return addTarget (con, 1d);
   }
    ...
    * @return the component used to keep track of this force target
    */
   public ConstraintForceTarget addTarget(BodyConnector con, double weight) {
      return addTarget (
   ...
   public ConstraintForceTarget addTarget(BodyConnector con, 
      ...
      return addTarget (con, targetLambda, 1d);
   }
    ...
    * @param source
    */
   protected boolean removeTarget (BodyConnector bodyCon) {
      int idx = -1;
      for (int i=0; i<myForceTargets.size(); i++) {
         if (myForceTargets.get(i).getConnector() == bodyCon) {
            idx = i;
            break;
         }
      }
   ...
   VectorNd collectAllWeights() {
      ...
      for (ConstraintForceTarget target : myForceTargets) {
         ...
         VectorNd subWeights = target.getSubWeights();
   ...
   public ArrayList<ConstraintForceTarget> getTargets () {
      ArrayList<ConstraintForceTarget> targets = new ArrayList<>();
      targets.addAll (myForceTargets);
      return targets;
   }

   public ArrayList<BodyConnector> getSources () {
      ArrayList<BodyConnector> sources = new ArrayList<>();
      for (ConstraintForceTarget t : myForceTargets) {
         sources.add (t.getConnector());
      }
      return sources;
   }

   // public ComponentList<ConstraintForceTarget> getForceTargets () {
   //    return myForceTargets;
   // }
   ...
   public void connectToHierarchy (CompositeComponent hcomp) {
      if (isInternal() && getParent() == hcomp && getController() != null) {
         ...
         tcon.myConstraintForceTerm = this;
      }
   }
   ...
   public void disconnectFromHierarchy(CompositeComponent hcomp) {
      if (isInternal() && getParent() == hcomp && getController() != null) {
         TrackingController tcon = (TrackingController)getParent();
         tcon.myConstraintForceTerm = null;
      }
   }
