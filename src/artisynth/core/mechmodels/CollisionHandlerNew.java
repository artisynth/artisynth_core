...
public class CollisionHandlerNew extends CollisionHandler {
   ...
   private LinkedHashMap<ContactPoint,ContactConstraint> myBilaterals0;
   private LinkedHashMap<ContactPoint,ContactConstraint> myBilaterals1;
   private ArrayList<ContactConstraint> myUnilaterals;
   int myMaxUnilaterals = 100;

   Collidable myCollidable0;
   Collidable myCollidable1;
   ...
   Method myMethod = myDefaultMethod;
   ...
   public Collidable getCollidable (int idx) {
      switch (idx) {
         case 0: return myCollidable0;
         case 1: return myCollidable1;
         default:
            throw new ArrayIndexOutOfBoundsException (
               "idx must be 0 or 1; is "+idx);
      }
   }
   ...
   public boolean isRigidBodyPair() {
      // XXX Semantics should be different now
      return myMethod == Method.CONTOUR_REGION;
   }         

   public CollisionHandlerNew () {
   ...
   protected boolean isRigid (Collidable col) {
      return (col instanceof RigidBody || col instanceof RigidMesh);
   }

   public CollisionHandlerNew (Collidable col0, Collidable col1, double mu) {
      this();
      myCollidable0 = col0;
      myCollidable1 = col1;
      if (isRigid(col0) && isRigid(col1)) {
         myMethod = Method.CONTOUR_REGION;
      }
   ...
   // protected ContactConstraint getContact (Vertex3d... vtxs) {
   //    myContactStub.myVtxs = vtxs;
   //    return myBilaterals.get(myContactStub);
   // }
   ...
   public double computeCollisionConstraints (double t) {
      ...
      boolean needContours = (myMethod==Method.CONTOUR_REGION);
   ...
   double setVertexFace (
      ...
      Collidable collidable0, Collidable collidable1) {
   ...
   boolean hashContactUsingFace (Collidable collidable0, Collidable collidable1) {
...
//            (myAttachedVertices1 != null ? myAttachedVertices1.size() : 0));
   ...
   */
   ...
   boolean hasLowDOF (Collidable collidable) {
      // XXX should formalize this better
      return isRigid (collidable);
   }

   double computeVertexPenetrationConstraints (
      ContactInfo info, Collidable collidable0, Collidable collidable1) {
      double maxpen = 0;
      ...
      if (info != null) {
         ...
         if (myMethod == Method.VERTEX_EDGE_PENETRATION) {
   ...
   public void getInitialAuxState (DataBuffer newData, DataBuffer oldData) {
      ...
      if (false) {
         if (oldData == null) {
            ...
            int numb = oldData.zget();
            int numu = oldData.zget();
            for (int i=0; i<numb+numu; i++) {
