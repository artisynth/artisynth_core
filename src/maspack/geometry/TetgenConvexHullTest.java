 ...
 * </pre>
 * will cause TetgenTessellator to be tested on a number of randomly
 * choosen input sets, with degenerate points added near
 * the edges and vertices of the convex hull.
 *
 * <p>The command
 * <pre>
...
public class TetgenConvexHullTest extends UnitTest {
   ...
   static int degeneracyTest = NO_DEGENERACY;
...
//    void explicitFaceCheck (TetgenTessellator hull, int[][] checkFaces)
//       throws Exception {
...
//    }
   ...
   void singleTest (double[] coords, int[][] checkFaces) {
      ...
      hull.buildFromPoints (coords); //, coords.length/3);
//       if (triangulate) {
//          hull.triangulate();
//       }
   ...
   double[] addDegeneracy (
      int type, double[] coords, TetgenTessellator hull) {
	   
      int numv = coords.length/3;
      int[] faces = hull.getHullFaces();
      double[] coordsx = new double[coords.length+faces.length];
      for (int i=0; i<coords.length; i++) {
         coordsx[i] = coords[i]; 
      }

      double[] lam = new double[3];
      double eps = hull.getCharacteristicLength()*1e-10;
   ...
   void degenerateTest (TetgenTessellator hull, double[] coords) {
      ...
      TetgenTessellator xhull = new TetgenTessellator();
      //xhull.setDebug (debugEnable);

      try {
         xhull.buildFromPoints (coordsx);
         //if (triangulate) {
         // xhull.triangulate();
         //}
      }
   ...
   /**
    * Runs a set of explicit and random tests on TetgenTessellator,
   ...
   /**
    * Runs timing tests on TetgenTessellator, and prints
