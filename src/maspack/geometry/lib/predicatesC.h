#ifndef _PREDICATESC_H_
#define _PREDICATESC_H_

#ifndef REAL
#define REAL double // this is important
#endif

/* BE CAREFUL, this must be set according to the
 * number of operations being performed on each
 * exactFloat. More operations will grow the
 * expansions. This can be (hopefully?) avoided
 * by using the '_zeroelim' versions of Shewchuck's
 * implementations.
 * 
 * Check here if Seg Faults are occuring.
 *
 ...
 * to be stored exactly in < 50 (2^11/53) doubles.
 */
#define MAXDOUBLES 100
/* AJL's code for neat encapsulation of data structure
 * Note that AJL's implementation use end = length - 1
 * whereas the modified shewchuck use end = length.
 */
typedef struct {
	int end;
...
/*
// When using this struct, must malloc MAXDOUBLESBIG
typedef struct {
	int end;
	double *doubles; 
} exactFloatBig; 
*/
// These functions are used in geoPred.c
REAL exactinit();
