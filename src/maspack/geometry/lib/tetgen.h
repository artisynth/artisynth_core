...
// John Lloyd: added this Feb 16, 2011 to fix bug where casting
// a point to long fails because the pointer is bigger than the long
#ifdef PTR_EXCEEDS_LONG // means that pointers are larger than longs
...
inline bool tetgenmesh::issymexist(triface* t) {
  tetrahedron *ptr = (tetrahedron *) 
    ((INT_PTR)(t->tet[t->loc]) & ~(INT_PTR)7l);
  return ptr != dummytet;
}
