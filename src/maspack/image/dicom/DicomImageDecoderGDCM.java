...
import gdcm.LookupTable.LookupTableType;
import gdcm.PhotometricInterpretation;
import gdcm.PhotometricInterpretation.PIType;
import gdcm.PixelFormat;
import gdcm.PixelFormat.ScalarType;
...
public class DicomImageDecoderGDCM implements DicomImageDecoder {
   ...
   public static byte[] GetAsByte(Image input) {
      ...
      if (pi.GetType() == PIType.MONOCHROME1) {
         ImageChangePhotometricInterpretation icpi =
            new ImageChangePhotometricInterpretation();
         icpi.SetInput(input);
         pi = new PhotometricInterpretation (PIType.MONOCHROME2);
         icpi.SetPhotometricInterpretation(pi);
         if (icpi.Change()) {
      ...
      try {
         ...
         if (ir.Read()) {
            ...
            if (pf.GetSamplesPerPixel() == 1) {
               if (pi.GetType() == PIType.MONOCHROME1
                   || pi.GetType() == PIType.MONOCHROME2) {
               
                  ScalarType stype = pf.GetScalarType();
                  if (pf.GetScalarType() == ScalarType.UINT8) {
                     pixelType = PixelType.UBYTE;
                  }
                  else if (pf.GetScalarType() == ScalarType.INT8) {
                     pixelType = PixelType.BYTE;
                  }
                  else if (pf.GetScalarType() == ScalarType.UINT12
                           || pf.GetScalarType() == ScalarType.UINT16) {
                     pixelType = PixelType.USHORT;
                  }
                  else if (stype == ScalarType.INT12
                           || pf.GetScalarType() == ScalarType.INT16) {
                     pixelType = PixelType.SHORT;
                  }
               }
               else if (pi.GetType() == PIType.PALETTE_COLOR) {
                  LookupTable lut = img.GetLUT();
                  long rl = lut.GetLUTLength(LookupTableType.RED);
                  byte[] rbuf = new byte[(int)rl];
                  long rl2 = lut.GetLUT(LookupTableType.RED, rbuf);
                  assert rl == rl2;
                  long gl = lut.GetLUTLength(LookupTableType.GREEN);
                  byte[] gbuf = new byte[(int)gl];
                  long gl2 = lut.GetLUT(LookupTableType.GREEN, gbuf);
                  assert gl == gl2;
                  long bl = lut.GetLUTLength(LookupTableType.BLUE);
                  byte[] bbuf = new byte[(int)bl];
                  long bl2 = lut.GetLUT(LookupTableType.BLUE, bbuf);
                  assert bl == bl2;
            ...
            double rescaleSlope = header.getDecimalValue (
               DicomTag.RESCALE_SLOPE, 1);
            double rescaleIntercept =
               header.getDecimalValue(DicomTag.RESCALE_INTERCEPT, 0);
         
            // System.out.println("Image: " + width + " x " + height + " x " +
            // slices);
            ...
            if (pixelType == PixelType.UBYTE) {
               ...
               for (int i=0; i<slices; ++i) {
                  DicomPixelBufferBase buff =
                     new UBytePixelBuffer(buffer, i*slicelen, slicelen);
               ...
               for (int i=0; i<slices; ++i) {
                  DicomPixelBufferBase buff =
                     new BytePixelBuffer(buffer, i*slicelen, slicelen);
                  // apply rescale slope/intercept
                  buff.setRescale(rescaleSlope, rescaleIntercept);
                  frames.add(buff);
               }
               ...
               for (int i=0; i<slices; ++i) {
                  DicomPixelBufferBase buff =
                     new RGBPixelBuffer(buffer, i*slicelen, slicelen);
                  buff.setRescale(rescaleSlope, rescaleIntercept);
                  frames.add(buff);
               }
            }
               ...
               for (int i=0; i<slices; ++i) {
                  DicomPixelBufferBase buff =
                     new UShortPixelBuffer(buffer, i*slicelen, slicelen);
                  buff.setRescale(rescaleSlope, rescaleIntercept);
                  frames.add(buff);
               }
            }
               ...
               for (int i=0; i<slices; ++i) {
                  DicomPixelBufferBase buff =
                     new ShortPixelBuffer(buffer, i*slicelen, slicelen);
                  buff.setRescale(rescaleSlope, rescaleIntercept);
                  frames.add(buff);
               }
            }
            ...
            System.err.println(
               "GDCM failed to read " + bin.getFile().getAbsolutePath());
            return false;
         }
      }
      catch (Exception e) {
