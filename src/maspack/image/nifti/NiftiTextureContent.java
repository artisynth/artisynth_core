...
public class NiftiTextureContent extends ReferenceCountedBase implements TextureContent {
   ...
   public NiftiTextureContent(NiftiImage image, NiftiPixelGenerator voxelator) {
      ...
      valid = new boolean[3];
      dirty = new boolean[3];
      internalStorage = image.getDataType();
   ...
   protected void updateBackingBuffer(int plane) {
      ...
      synchronized (textureImage) {
         ...
         switch (plane) {
            case COL_ROW_PLANE:
               image.getPixels (0, 1, rect.width(), 0, 1, rect.height(), slice, 1, 1,
                  time, scanline, 0, voxelator, textureImage);
               break;
            case COL_SLICE_PLANE:
               image.getPixels (0, 1, rect.width(), row, 1, 1, 0, 1, rect.height (), 
                  time, 0, scanline, voxelator, textureImage);
               break;
            case ROW_SLICE_PLANE:
               image.getPixels (col, 1, 1, 0, 1, rect.width(), 0, 1, rect.height(),
                                 time, 0, scanline, voxelator, textureImage);
                break;
         }
         valid[plane] = true;   
   ...
   public void showTexture() {
      ...
      if (textureFrame == null) {
         textureFrame = new JFrame ("NiftiTextureContent texture");
   ...
   public NiftiTextureContent acquire () {
      return (NiftiTextureContent)super.acquire ();
   }
