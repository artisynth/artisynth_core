...
//         "automatically compute number of divisions", true);
   ...
   void setViewer (GLViewer viewer) {
      if (myViewer != viewer) {
         if (myDragger != null) {
            if (myViewer != null) {
               myViewer.myDraggers.remove (myDragger);
            }
            if (viewer != null) {
               viewer.myDraggers.add (myDragger);
            }
         }
         myViewer = viewer;
      }
   ...
   public void setDragger (DraggerType type) {
      if (type != getDragger()) {
         ...
         if (myViewer != null) {
            if (myDragger != null) {
               myViewer.myDraggers.add (myDragger);
            }
            myViewer.repaint();
         }
      }
   ...
   void getPlaneValues (double[] vals) {
      RigidTransform3d X = XGridToWorld;
      vals[0] = -X.R.m02;
      vals[1] = -X.R.m12;
      vals[2] = -X.R.m22;
   ...
   private double computeFocalPoint (
      RigidTransform3d XGridToEye, Point3d focus, GLRenderer renderer) {
   ...
   private void drawGrid (GLRenderer renderer) {
      if (myMinSize == 0) {
         return;
      }
      ...
      for (int i = 0; i < xcnt; i++) {
         ...
         int ii = y_axis_i - i;
         if (ii == 0 && myYAxisRGB != null) {
            renderer.setColor (myYAxisRGB);
         }
         else if (ii%numDivisions == 0) {
            renderer.setColor (myMajorRGB);
         }
         else {
            renderer.setColor (minorRGB);
         }
         gl.glVertex3d (x, ymin, 0);
         gl.glVertex3d (x, ymax, 0);
      }
      for (int j = 0; j < ycnt; j++) {
         double y = ymin + j * minorSize;
         int jj = x_axis_j - j;
         if (jj == 0 && myXAxisRGB != null) {
            renderer.setColor (myXAxisRGB);
         }
         ...
         gl.glVertex3d (xmin, y, 0);
         gl.glVertex3d (xmax, y, 0);
      }
      gl.glEnd();
      gl.glPopMatrix();
