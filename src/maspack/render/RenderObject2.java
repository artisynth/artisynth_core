...
//   }
   ...
   /**
    * Lock the object to prevent any other threads from
    * concurrently writing to this.
    */
   public void lock() {
      readLock();
   }
   ...
   /**
    * Unlock the object to allow other threads to write.
    */
   public void unlock() {
      readUnlock();
   }
   ...
   protected void readLock() {
      System.err.println (this + " read locked  (" + Thread.currentThread () + ")");
      readWriteLock.readLock ().lock ();
   }
   
   protected void readUnlock() {
      System.err.println (this + " read unlocked  (" + Thread.currentThread () + ")");
      readWriteLock.readLock ().unlock ();
   }
   
   protected void writeLock() {
      System.err.println (this + " write locked  (" + Thread.currentThread () + ")");
      readWriteLock.writeLock ().lock ();
   }
   
   protected void writeUnlock() {
      System.err.println (this + " write unlocked  (" + Thread.currentThread () + ")");
   ...
   public float[] getPosition(int pidx) {
      readLock ();  // prevent potential crash on positions
    ...
    * be modified.  For safe concurrent access, this object's lock
    ...
    * must be called.  For safe concurrent access, this object's lock
    * should first be acquired.
    * @see #lock()
    */
    ...
    * 
    * For safe concurrent access, this object's lock
    * should first be acquired.
   ...
   public int numTriangles(int tgroup) {
      readLock ();
      int s = triangles.get(tgroup).size();
      readUnlock ();
      return s;
   }
   ...
   @Override
   protected RenderObject2 clone()  {

      RenderObject2 r = new RenderObject2();
