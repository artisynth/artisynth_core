...
public class CGSolverTest {
   ...
   public void test() {
      ...
      for (int i = 0; i < 10; i++) {
         ...
         if (!solver.solve (x, M, b, eps, size * size, I)) {
            throw new TestException (
               "No convergence, identity preconditioner: error="
               + solver.getRelativeResidual());
         }
         if (numIter != solver.getNumIterations()) {
            throw new TestException (
               "Different iteration count with identity preconditioner: "
               + solver.getNumIterations() + " vs. " + numIter);
         }
         LU.inverse (Minv);
         x.setZero();
         if (!solver.solve (x, M, b, eps, size * size, Minv)) {
            throw new TestException (
               "No convergence, inverse preconditioner: error="
               + solver.getRelativeResidual());
         }
         if (solver.getNumIterations() > 2) {
            throw new TestException (
               "Num iterations > 2 with inverse preconditioner");
         }
   ...
   public static void main (String[] args) {
      ...
      CGSolverTest tester = new CGSolverTest();
      try {
         if (dotiming) {
            tester.timing();
         }
